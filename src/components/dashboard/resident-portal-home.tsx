...
type Payload = ResidentPortalHomeState;

function reasonCopy(reason?: string) {
  if (reason === "invalid") return "Hyresavtal, ämne och en tydlig beskrivning krävs.";
  if (reason === "forbidden") return "Du saknar behörighet att skapa boendeärenden.";
  if (reason === "missing") return "Det aktiva hyresavtalet hittades inte.";
  if (reason === "error") return "Kunde inte skapa boendeärendet. Försök igen.";
  return "";
}
...
export function ResidentPortalHome({ initial, createdReference, reason }: Props) {
  ...
  const [success, setSuccess] = useState(
    createdReference ? `Ärendet har skapats med referens ${createdReference}.` : "",
  );
  const [form, setForm] = useState({
    leaseId: initial.leases[0]?.id || "",
    category: "other",
    priority: "normal",
    subject: "",
    message: "",
            ...
            <form method="post" action="/api/resident-portal" onSubmit={submit} className="space-y-4">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <label className="block space-y-1.5">
                  ...
                  <select name="category" value={form.category} onChange={(event) => setForm({ ...form, category: event.target.value })} className={premiumFieldClass}>
                    {Object.entries(categoryLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}
                  </select>
                </label>
                <label className="block space-y-1.5">
                  <span className="text-xs font-semibold text-ink-700">Prioritet</span>
                  <select name="priority" value={form.priority} onChange={(event) => setForm({ ...form, priority: event.target.value })} className={premiumFieldClass}>
              ...
              <label className="block space-y-1.5">
                ...
                <input required name="subject" maxLength={200} placeholder="Exempel: Läckage under diskbänk" value={form.subject} onChange={(event) => setForm({ ...form, subject: event.target.value })} className={premiumFieldClass} />
              </label>
              <label className="block space-y-1.5">
                <span className="text-xs font-semibold text-ink-700">Beskrivning</span>
                <textarea required name="message" minLength={10} maxLength={5000} placeholder="Beskriv problemet, när det började och om någon akut risk finns." value={form.message} onChange={(event) => setForm({ ...form, message: event.target.value })} className={premiumTextareaClass} />
              </label>
              <button type="submit" disabled={saving || !form.leaseId} className={`${premiumPrimaryButtonClass} w-full`}>
