...
import { EmptyState, InlineAlert, PageHeader, premiumFieldClass, premiumPrimaryButtonClass, premiumSecondaryButtonClass, premiumTextareaClass } from "@/components/dashboard/premium-ui";
...
function KpiCard({ icon: Icon, label, value, hint, tone = "green" }: { icon: LucideIcon; label: string; value: number | string; hint: string; tone?: "green" | "red" | "amber" }) {
  ...
  return <article className="rounded-2xl border border-sand-200 bg-white p-5 shadow-premium-sm">
    <div className="flex items-start gap-4">
      ...
      <div className="min-w-0"><p className="text-[11px] font-medium text-ink-550">{label}</p><p className="mt-1 font-display text-[30px] font-semibold leading-none tracking-[-0.04em] text-ink-950">{value}</p><p className={`mt-3 text-[13px] font-semibold ${hintTone}`}>{hint}</p></div>
    </div>
  </article>;
}
...
export default function FelanmalanPage() {
  ...
  return <div className="space-y-4 sm:space-y-5">
    <PageHeader
      catalog="arenden"
      eyebrow="Drift · Ärenden"
      title="Ärenden"
      description="Översikt och handläggning av kund- och driftärenden med AI-klassificering, tilldelning, karta och export."
      records={loading ? "Hämtar…" : pagination.total.toLocaleString("sv-SE")}
      action={(
        <div className="flex flex-wrap gap-2">
          {permissions.canExport ? <a href="/api/tickets/export" className={premiumSecondaryButtonClass}><Download className="mr-2 h-4 w-4" />Exportera</a> : null}
          {permissions.canManage ? <button type="button" onClick={() => setCreateOpen(true)} className={premiumPrimaryButtonClass}>Nytt ärende</button> : null}
        </div>
      )}
    />
    ...
    <section className="rounded-2xl border border-sand-200 bg-white p-3 shadow-premium-sm">
      <div className="grid gap-2 md:grid-cols-2 xl:grid-cols-[minmax(260px,1.6fr)_120px_120px_130px_150px_140px_160px_auto]">
        ...
        {hasFilters ? <button type="button" onClick={clearFilters} className="h-10 px-2 text-[13px] font-semibold text-petroleum-700 hover:text-petroleum-900">Rensa filter</button> : <span />}
      </div>
    </section>
