...
import { PropertyStructureManager } from "@/components/properties/property-structure-manager";
...
export default async function PropertyDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const openTickets = await db.ticket.count({ where: { property_id: property.id, status: { not: "closed" }, ...tenantWhere(user) } });
  ...
  return (
    <div className="animate-fade-in-soft space-y-6">
          ...
          <div className="relative flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <div className="mb-3 flex flex-wrap items-center gap-2">
                <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-petroleum-600">Fastighetskort</p>
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-2">
        <section className="overflow-hidden rounded-2xl border border-sand-200 bg-white shadow-premium-sm">
          <div className="border-b border-sand-200 px-6 py-5 sm:px-7"><h2 className="text-xl font-semibold text-ink-950">Byggnader</h2><p className="mt-1 text-sm text-ink-500">Struktur, adresser och antal registrerade objekt.</p></div>
        ...
        <section className="overflow-hidden rounded-2xl border border-sand-200 bg-white shadow-premium-sm">
          <div className="border-b border-sand-200 px-6 py-5 sm:px-7"><h2 className="text-xl font-semibold text-ink-950">Lägenheter och lokaler</h2><p className="mt-1 text-sm text-ink-500">Objektsregister för boende, lokaler och tekniska ytor.</p></div>
      ...
      <PropertyStructureManager propertyId={property.id} buildings={property.buildings} units={property.units} />
