    ...
    try {
      ...
      setDrafts((current) => {
        ...
        for (const job of eligible) {
          next[job.jobId] ??= { note: "", externalId: job.externalId || "" };
        }
        return next;
      });
    } catch (value) {
        ...
        {visible ? (
          <div className="space-y-4">
            {jobs.map((job) => {
              ...
              return (
                <article key={job.jobId} className="rounded-2xl border border-amber-200 bg-amber-50/60 p-5">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-amber-900/80">
                        ...
                        {job.processingStartedAt ? ` · startad ${dateTime.format(new Date(job.processingStartedAt))}` : ""}
                      </p>
                    </div>
                    <span className="rounded-full border border-amber-200 bg-white px-3 py-1 text-xs font-semibold text-amber-900">Bearbetas · stale</span>
                  </div>
                  ...
                  <div className="mt-4 grid gap-3 lg:grid-cols-2">
                    <label className="space-y-2 text-sm lg:col-span-2">
                      ...
                      <textarea
                        ...
                        maxLength={1000}
                        rows={3}
                        placeholder="Exempel: Kontrollerad i Fortnox 20:42. Fakturan finns med externt nummer ..."
                    ...
                    <div className="flex flex-wrap items-end gap-2">
                      <button
                        type="button"
                        disabled={saving || draft.note.trim().length < 10}
                        onClick={() => void reconcile(job, "sent")}
                      ...
                      <button
                        type="button"
                        disabled={saving || draft.note.trim().length < 10}
                        onClick={() => void reconcile(job, "failed")}
                        className="inline-flex items-center gap-2 rounded-xl border border-red-200 bg-white px-4 py-2.5 text-sm font-semibold text-red-700 hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-50"
                      >
