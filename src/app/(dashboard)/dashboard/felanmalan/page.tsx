...
function KpiCard({ icon: Icon, label, value, hint, tone = "green" }: { icon: LucideIcon; label: string; value: number | string; hint: string; tone?: "green" | "red" | "amber" }) {
  const iconTone = tone === "red" ? "bg-danger-50 text-danger-600" : tone === "amber" ? "bg-warning-50 text-warning-700" : "bg-petroleum-50 text-petroleum-700";
  return <div className="flex items-start gap-3 px-5 py-4">
    <span className={`mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-lg ${iconTone}`}><Icon className="h-4 w-4" strokeWidth={1.8} /></span>
    <div className="min-w-0">
      <dt className="text-[10px] font-semibold uppercase tracking-[0.09em] text-ink-450">{label}</dt>
      <dd className="mt-1.5 font-display text-[26px] font-semibold leading-none tracking-[-0.04em] text-ink-950">{value}</dd>
      <p className="mt-1.5 text-[10px] leading-4 text-ink-400">{hint}</p>
    </div>
  </div>;
}
...
export default function FelanmalanPage() {
  ...
  return <div className="space-y-4 sm:space-y-5">
    ...
    <section className="rounded-2xl border border-sand-200 bg-white shadow-premium-sm" aria-label="Nyckeltal för ärenden">
      <dl className="grid divide-y divide-sand-100 sm:grid-cols-2 sm:divide-y-0 xl:grid-cols-4 xl:divide-x">
