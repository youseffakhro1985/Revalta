  ...
  return (
    <>
      <button
        ...
        className="hidden h-10 min-w-[280px] items-center gap-3 rounded-lg border border-sand-200 bg-white px-3 text-left text-[12px] text-ink-500 shadow-[0_1px_2px_rgba(17,34,31,0.04)] transition hover:border-sand-300 lg:flex"
      >
      ...
      {open ? (
        <div className="fixed inset-0 z-[100] flex items-start justify-center bg-ink-950/25 px-4 pt-[12vh] backdrop-blur-[2px]" role="dialog" aria-modal="true" aria-label="Sök i Revalta" onMouseDown={() => setOpen(false)}>
          <div className="w-full max-w-2xl overflow-hidden rounded-2xl border border-sand-200 bg-[#FAFAF8] shadow-[0_24px_80px_rgba(17,34,31,0.18)]" onMouseDown={(event) => event.stopPropagation()}>
            <div className="flex items-center gap-3 border-b border-sand-200 px-5">
              ...
              <input
                ...
                className="h-16 flex-1 bg-transparent text-[15px] text-ink-900 outline-none placeholder:text-ink-500"
              />
              <button type="button" onClick={() => setOpen(false)} aria-label="Stäng sökning" className="rounded-lg p-2 text-ink-500 hover:bg-sand-100 hover:text-ink-700">
                <X className="h-4 w-4" />
              </button>
            </div>
            ...
            <div className="max-h-[430px] overflow-y-auto p-3">
              {query.trim().length < 2 ? (
                <div className="px-4 py-10 text-center">
                  ...
                  <p className="mt-1 text-[12px] text-ink-500">Skriv minst två tecken för att hitta fastigheter, ärenden, användare och hyresparter.</p>
                </div>
              ) : loading ? (
                <p className="px-4 py-10 text-center text-[12px] text-ink-500">Söker...</p>
              ) : results.length === 0 ? (
                <p className="px-4 py-10 text-center text-[12px] text-ink-500">Inga träffar hittades.</p>
              ) : (
                    ...
                    return (
                      <Link key={`${result.type}-${result.id}`} href={result.href} onClick={() => setOpen(false)} className="flex items-center gap-3 rounded-xl px-3 py-3 transition hover:bg-white hover:shadow-[0_1px_3px_rgba(17,34,31,0.06)]">
                        ...
                        <span className="min-w-0 flex-1">
                          ...
                          <span className="mt-0.5 block truncate text-[11px] text-ink-500">{result.subtitle}</span>
                        </span>
                      </Link>
                    );
                  })}
