  ...
  return (
    <section className="overflow-hidden rounded-2xl border border-sand-200 bg-white shadow-premium-sm" aria-labelledby="service-alert-center-title">
      ...
      <div className="grid gap-3 border-b border-sand-100 px-5 py-4 sm:grid-cols-4">
        ...
        <div><p className="text-xs font-semibold uppercase tracking-wide text-ink-500">Kritiska</p><p className="mt-1 text-xl font-semibold text-danger-800">{data?.summary.critical || 0}</p></div>
        <div><p className="text-xs font-semibold uppercase tracking-wide text-ink-500">Lösta</p><p className="mt-1 text-xl font-semibold text-success-800">{data?.summary.resolved || 0}</p></div>
      </div>
        ...
        {openAlerts.map((item) => (
          <article key={item.id} className={`grid gap-4 px-5 py-5 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center ${item.severity === "critical" ? "bg-danger-50/35" : "bg-warning-50/35"}`}>
            <div className="flex min-w-0 items-start gap-3">
              ...
              <div>
                <div className="flex flex-wrap items-center gap-2"><h3 className="font-semibold text-ink-950">{item.title}</h3><span className={`rounded-full px-2 py-0.5 text-[11px] font-semibold ${item.severity === "critical" ? "bg-danger-100 text-danger-800" : "bg-warning-100 text-warning-800"}`}>{item.severity === "critical" ? "Kritisk" : "Varning"}</span>{item.acknowledged ? <span className="rounded-full bg-sand-100 px-2 py-0.5 text-[11px] font-semibold text-ink-600">Kvitterad</span> : null}</div>
                ...
                {item.source === "legacy" ? (
                  <p className="mt-2 text-xs font-medium text-warning-800">Äldre larm – kör backfill innan det kan kvitteras.</p>
                ) : null}
              </div>
            </div>
