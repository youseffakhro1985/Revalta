    ...
    <div className="space-y-8">
      ...
      <section className={`grid gap-6 ${canManage ? "xl:grid-cols-[380px_1fr]" : "grid-cols-1"}`}>
        ...
        <form onSubmit={submit} className="space-y-4 rounded-2xl border border-sand-200 bg-white p-6 shadow-premium-sm">
          ...
          <select value={form.resource} onChange={(e) => setForm({ ...form, resource: e.target.value })} className="w-full rounded-xl border border-sand-200 px-4 py-3 text-sm">
                ...
                <article key={booking.id} className="space-y-3 p-5">
                  ...
                  {canManage && editingId === booking.id ? (
                    <div className="grid gap-3 rounded-xl border border-sand-200 bg-sand-50 p-4 md:grid-cols-2">
                      ...
                      <input value={editForm.residentName} onChange={(e) => setEditForm({ ...editForm, residentName: e.target.value })} className="rounded-xl border border-sand-200 bg-white px-3 py-2 text-sm" placeholder="Boendes namn" />
                      <input value={editForm.unit} onChange={(e) => setEditForm({ ...editForm, unit: e.target.value })} className="rounded-xl border border-sand-200 bg-white px-3 py-2 text-sm" placeholder="Lägenhet/lokal" />
                      <input type="datetime-local" value={editForm.start} onChange={(e) => setEditForm({ ...editForm, start: e.target.value })} className="rounded-xl border border-sand-200 bg-white px-3 py-2 text-sm" />
                      <input type="datetime-local" value={editForm.end} onChange={(e) => setEditForm({ ...editForm, end: e.target.value })} className="rounded-xl border border-sand-200 bg-white px-3 py-2 text-sm" />
                      <textarea value={editForm.note} onChange={(e) => setEditForm({ ...editForm, note: e.target.value })} className="min-h-20 rounded-xl border border-sand-200 bg-white px-3 py-2 text-sm md:col-span-2" placeholder="Anteckning" />
