...
export async function getLeaseHandoverReport(companyId: string, leaseId: string) {
  ...
  const [handoverEvent, inspectionEvent, workOrders, audit] = await Promise.all([
    db.integrationEvent.findFirst({ where: { company_id: companyId, type: HANDOVER_EVENT_TYPE, recipient: leaseId }, orderBy: { created_at: "desc" } }),
    db.integrationEvent.findFirst({ where: { company_id: companyId, type: inspectionRecordEventType, recipient: leaseId }, orderBy: { created_at: "desc" } }),
    readInspectionWorkOrders(companyId, leaseId),
    db.auditLog.findMany({ where: { company_id: companyId, entity_id: leaseId, entity_type: { in: ["lease_handover", "lease_inspection"] } }, orderBy: { created_at: "desc" }, take: 100, select: { id: true, action: true, created_at: true, metadata: true } }),
  ]);

  const handover = handoverEvent?.payload && typeof handoverEvent.payload === "object" ? handoverEvent.payload as unknown as LeaseHandoverPayload : null;
  const inspection = inspectionEvent?.payload && typeof inspectionEvent.payload === "object" ? inspectionEvent.payload as unknown as LeaseInspectionRecord : null;
