...
PR #426's last inspected head `1c752cecd364b5089c0783fc580b2495ba5b1209` has successful Revalta CI (206 files / 1,340 tests), CodeQL security check and Vercel statuses. Preview Browser E2E fails closed because isolation and verified credentials are absent. Inspection found that CI/CodeQL still checked out GitHub's generated merge commit; the current change explicitly checks out and verifies the PR head and uploads CodeQL results against that head. Its new SHA needs fresh checks; historical statuses are not substituted for them.

The current #426 change also requires real desktop/mobile navigation, successful property/search API responses, an active verified owner in the expected synthetic company (`E2E_VERIFIED_COMPANY_ID`), a working dashboard heading, interactive Command Center at all six widths, logout followed by protected UI/API rejection, and unchanged SHA/deployment at the end. Runners suppress raw browser/request exceptions that may contain credentials. Local policy/contract verification: **49 tests passed**. Authenticated Preview execution is **still blocked**, and the company assertion is not proof of database isolation.
