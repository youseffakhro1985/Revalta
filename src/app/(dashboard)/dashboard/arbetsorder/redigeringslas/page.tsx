...
export default function WorkOrderEditLocksPage() {
  ...
  useEffect(() => {
    ...
    window.setTimeout(() => document.getElementById("las-sok")?.focus(), 0);
  }, [loading]);
  ...
  return <div className="space-y-8">
    ...
    <Panel title="Driftläge" description="Vyn uppdateras automatiskt var 30:e sekund. Utgångna lås rensas innan resultatet visas.">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="relative w-full max-w-xl"><Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-ink-500" /><input id="las-sok" autoFocus value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Sök arbetsorder, fastighet eller redigerare" aria-label="Sök arbetsorder, fastighet eller redigerare" className={`${premiumFieldClass} pl-10`} /></div>
