...
for (const [fragment, message] of [
  ...
  ["^([0-9a-f]{64})  ([^\\r\\n]+)\\n$", "Checksum sidecars must use lowercase LF-only canonical bytes"],
  ...
  ["Buffer.from(attestationBytes)", "Snapshot verification must copy attestation bytes"],
  ["Buffer.from(checksumBytes ?? [])", "Snapshot verification must copy checksum bytes"],
  ...
  ["Promise.all", "File verification must capture both evidence files together"],
  ["readFile(resolvedAttestationPath)", "File verification must read the artifact once"],
  ["readFile(resolvedChecksumPath)", "File verification must read the sidecar once"],
