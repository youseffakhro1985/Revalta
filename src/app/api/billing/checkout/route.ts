...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!isStripeReady(plan)) {
      ...
      // Billing is a money flow: production must never return a mock checkout,
      // even if a preview/mock environment flag leaked into production.
    ...
    // The Stripe session is an external side effect and cannot be rolled back.
    // Post-session audit/telemetry must therefore be best-effort so a transient
    // local logging failure does not return a false 500 and encourage duplicate
    // checkout-session creation on retry.
    try {
