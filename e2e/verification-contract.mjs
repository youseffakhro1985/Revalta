...
export function validateOwnerUnknownMaintenancePlanWorkOrderNotFound(status, body) {
  if (
    status !== 404 ||
    body?.success === true ||
    body?.error !== "Fastigheten hittades inte" ||
    ...
    throw new Error("Verified owner unknown maintenance-plan work-order did not stay not found");
  }
}
...
export function validateOwnerUnknownWorkOrderSlaNotFound(status, body) {
  if (
    status !== 404 ||
    body?.success === true ||
    body?.error !== "Arbetsordern hittades inte" ||
    ...
    throw new Error("Verified owner unknown work-order SLA did not stay not found");
  }
}

export function validateOwnerUnknownWorkOrderEditLockNotFound(status, body) {
  if (
    status !== 404 ||
    body?.success === true ||
    body?.error !== "Arbetsordern hittades inte" ||
    ...
    throw new Error("Verified owner unknown work-order edit-lock did not stay not found");
  }
}

export function validateOwnerUnknownWorkOrderInvoiceBasisNotFound(status, body) {
  if (
    status !== 404 ||
    body?.success === true ||
    body?.error !== "Arbetsordern hittades inte" ||
    ...
    throw new Error("Verified owner unknown work-order invoice-basis did not stay not found");
  }
}

export function validateOwnerUnknownWorkOrderProfitabilityNotFound(status, body) {
  if (
    status !== 404 ||
    body?.success === true ||
    body?.error !== "Arbetsordern hittades inte" ||
    ...
    throw new Error("Verified owner unknown work-order profitability did not stay not found");
  }
}

export function validateOwnerUnknownWorkOrderInvoiceIntegrationNotFound(status, body) {
  if (
    status !== 404 ||
    body?.success === true ||
    body?.error !== "Arbetsordern hittades inte" ||
    ...
    throw new Error("Verified owner unknown work-order invoice-integration did not stay not found");
  }
}

export function validateOwnerUnknownWorkOrderTransitionsNotFound(status, body) {
  if (
    status !== 404 ||
    body?.success === true ||
    body?.error !== "Arbetsordern hittades inte" ||
    ...
    throw new Error("Verified owner unknown work-order transitions did not stay not found");
  }
}

export function validateOwnerUnknownWorkOrderInvoiceBasisGetNotFound(status, body) {
  if (
    status !== 404 ||
    body?.success === true ||
    body?.error !== "Arbetsordern hittades inte" ||
    ...
    throw new Error("Verified owner unknown work-order invoice-basis GET did not stay not found");
  }
}
...
export function validateOwnerUnknownWorkOrderProfitabilityGetNotFound(status, body) {
  if (
    status !== 404 ||
    body?.success === true ||
    body?.error !== "Arbetsordern hittades inte" ||
    ...
    throw new Error("Verified owner unknown work-order profitability GET did not stay not found");
  }
}

export function validateOwnerUnknownWorkOrderInvoiceIntegrationGetNotFound(status, body) {
  if (
    status !== 404 ||
    body?.success === true ||
    body?.error !== "Arbetsordern hittades inte" ||
    ...
    throw new Error("Verified owner unknown work-order invoice-integration GET did not stay not found");
  }
}

export function validateOwnerUnknownWorkOrderAssetOptionsNotFound(status, body) {
  if (
    status !== 404 ||
    body?.success === true ||
    body?.error !== "Arbetsordern hittades inte" ||
    ...
    throw new Error("Verified owner unknown work-order asset-options did not stay not found");
  }
}

export function validateOwnerUnknownWorkOrderAttestationNotFound(status, body) {
  if (
    status !== 404 ||
    body?.success === true ||
    body?.error !== "Arbetsordern hittades inte" ||
    ...
    throw new Error("Verified owner unknown work-order attestation did not stay not found");
  }
}
...
export function validateOwnerUnknownWorkOrderDocumentItemNotFound(status, body) {
  if (
    status !== 404 ||
    body?.success === true ||
    body?.error !== "Arbetsordern hittades inte" ||
    ...
    throw new Error("Verified owner unknown work-order document item did not stay not found");
  }
}

export function validateOwnerUnknownTicketOperationsGetNotFound(status, body) {
  if (
    status !== 404 ||
    body?.success === true ||
    body?.error !== "Ärendet hittades inte" ||
    ...
    throw new Error("Verified owner unknown ticket operations did not stay not found");
  }
}

export function validateOwnerUnknownTicketOperationsPostNotFound(status, body) {
  if (
    status !== 404 ||
    body?.success === true ||
    body?.error !== "Ärendet hittades inte" ||
    ...
    throw new Error("Verified owner unknown ticket operations write did not stay not found");
  }
}
...
export function validateOwnerUnknownTicketOperationsDeleteNotFound(status, body) {
  if (
    status !== 404 ||
    body?.success === true ||
    body?.error !== "Ärendet hittades inte" ||
    ...
    throw new Error("Verified owner unknown ticket operations delete did not stay not found");
  }
}
...
export function validateOwnerUnknownLeaseHandoverPutNotFound(status, body) {
  if (
    status !== 404 ||
    body?.success === true ||
    body?.error !== "Avtalet hittades inte" ||
    ...
    throw new Error("Verified owner unknown lease handover write did not stay not found");
  }
}
...
export function validateOwnerUnknownLeaseInspectionItemsPutNotFound(status, body) {
  if (
    status !== 404 ||
    body?.success === true ||
    body?.error !== "Avtalet hittades inte" ||
    ...
    throw new Error("Verified owner unknown lease inspection items write did not stay not found");
  }
}

export function validateOwnerUnknownLeaseInspectionItemWorkOrdersGetNotFound(status, body) {
  if (
    status !== 404 ||
    body?.success === true ||
    body?.error !== "Avtalet hittades inte" ||
    ...
    throw new Error("Verified owner unknown lease inspection-item work-orders did not stay not found");
  }
}
...
export function validateOwnerUnknownLeaseInspectionWorkOrdersReconcileNotFound(status, body) {
  if (
    status !== 404 ||
    body?.success === true ||
    body?.error !== "Avtalet hittades inte" ||
    ...
    throw new Error("Verified owner unknown lease inspection work-order reconcile did not stay not found");
  }
}
...
export function validateOwnerUnknownPropertyComponentsNotFound(status, body) {
  if (
    status !== 404 ||
    body?.success === true ||
    ...
    throw new Error("Verified owner unknown property components did not stay not found");
  }
}
...
export function validateOwnerUnknownPublicTicketNotFound(status, body) {
  if (
    status !== 404 ||
    body?.success === true ||
    body?.ticket ||
    ...
    throw new Error("Verified owner unknown public ticket did not stay not found");
  }
}

export function validateOwnerUnknownPublicTicketCommentNotFound(status, body) {
  if (
    status !== 404 ||
    body?.success === true ||
    body?.error !== "Ärendet hittades inte. Kontrollera referensnummer och e-post." ||
    ...
    throw new Error("Verified owner unknown public ticket comment did not stay not found");
  }
}

export function validateOwnerUnknownPublicTicketAttachmentNotFound(status, body) {
  if (
    status !== 404 ||
    body?.success === true ||
    body?.error !== "Ärendet hittades inte. Kontrollera referensnummer och e-post." ||
    ...
    throw new Error("Verified owner unknown public ticket attachment did not stay not found");
  }
}
...
export function validateOwnerUnknownResidentPortalTicketCommentNotFound(status, body) {
  if (
    status !== 404 ||
    body?.success === true ||
    body?.comment ||
    ...
    throw new Error("Verified owner unknown resident portal ticket comment did not stay not found");
  }
}
...
export function validateOwnerUnknownPropertyMaintenancePlanGovernanceNotFound(status, body) {
  if (
    status !== 404 ||
    body?.success === true ||
    Array.isArray(body?.plans) ||
    ...
    throw new Error("Verified owner unknown property maintenance plan governance did not stay not found");
  }
}

export function validateOwnerUnknownPropertyMaintenancePlanActionNotFound(status, body) {
  if (
    status !== 404 ||
    body?.success === true ||
    body?.error !== "Fastigheten hittades inte" ||
    ...
    throw new Error("Verified owner unknown property maintenance plan action did not stay not found");
  }
}
...
export function validateOwnerUnknownPropertyComponentMaintenanceSettingsNotFound(status, body) {
  if (
    status !== 404 ||
    body?.success === true ||
    body?.settings ||
    ...
    throw new Error("Verified owner unknown property component maintenance settings did not stay not found");
  }
}
...
export function validateOwnerUnknownPropertyComponentActionsNotFound(status, body) {
  if (
    status !== 404 ||
    body?.success === true ||
    body?.event ||
    ...
    throw new Error("Verified owner unknown property component actions did not stay not found");
  }
}
