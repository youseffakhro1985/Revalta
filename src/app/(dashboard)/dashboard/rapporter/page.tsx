...
function Metric({ icon: Icon, label, value, detail, href, alert }: MetricProps) {
  return (
    ...
    >
      ...
      <p className={`mt-2 text-xs leading-4 ${alert ? "text-danger-700" : "text-ink-500"}`}>{detail}</p>
    </Link>
  );
}
  ...
  return (
    <div className="space-y-5 pb-4">
      <header className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-petroleum-700">Analys & beslutsstöd / Rapporter</p>
      ...
      <nav className="print:hidden grid gap-2 sm:grid-cols-2 xl:grid-cols-4" aria-label="Rapportgenvägar">
        {[
          ...
          <Link key={title} href={href} className="group flex items-center justify-between rounded-2xl border border-sand-200 bg-surface-subtle px-4 py-3 transition hover:border-petroleum-200 hover:bg-white">
            <div><p className="text-xs font-semibold text-ink-800">{title}</p><p className="mt-0.5 text-xs text-ink-500">{description}</p></div>
            <ArrowRight className="h-4 w-4 text-ink-300 transition group-hover:translate-x-0.5 group-hover:text-petroleum-700" />
          </Link>
        ))}
      </nav>
      ...
      <section className="grid gap-4 xl:grid-cols-[1.25fr_0.75fr]">
        <article className="rounded-2xl border border-sand-200 bg-white p-5 shadow-premium-sm">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.14em] text-petroleum-700">Operativ utveckling</p>
              <h2 className="mt-1 font-display text-[19px] font-semibold text-ink-950">Ärenden och slutförda arbetsorder</h2>
            </div>
            <p className="text-xs text-ink-500">{dateLabel.format(currentStart)} – {dateLabel.format(now)}</p>
          </div>
          <div className="mt-4 flex gap-4 text-xs text-ink-500"><span className="inline-flex items-center gap-1.5"><span className="h-2 w-2 rounded-full bg-petroleum-800" />Nya ärenden</span><span className="inline-flex items-center gap-1.5"><span className="h-2 w-2 rounded-full bg-sand-500" />Slutförda arbetsorder</span></div>
            ...
            {trendBuckets.map((bucket) => (
              <div key={bucket.start.toISOString()} className="flex h-full flex-col justify-end">
                <div className="mb-2 flex min-h-6 items-end justify-center gap-1.5">
                  <span className="text-xs font-semibold text-petroleum-800">{bucket.tickets || ""}</span>
                  <span className="text-xs font-semibold text-ink-500">{bucket.completedOrders || ""}</span>
                </div>
                ...
                <p className="mt-2 truncate text-center text-xs capitalize text-ink-400">{bucket.label}</p>
              </div>
            ))}
          </div>
          <div className="mt-4 grid gap-3 sm:grid-cols-3">
            <div className="rounded-xl bg-surface-subtle px-3.5 py-3"><p className="text-xs text-ink-500">Genomsnittlig åtgärdstid</p><p className="mt-1 text-sm font-semibold text-ink-900">{formatDays(averageResolution)}</p></div>
            <div className="rounded-xl bg-surface-subtle px-3.5 py-3"><p className="text-xs text-ink-500">Aktiva arbetsorder</p><p className="mt-1 text-sm font-semibold text-ink-900">{activeOrders.length}</p></div>
            <div className="rounded-xl bg-surface-subtle px-3.5 py-3"><p className="text-xs text-ink-500">Akuta öppna ärenden</p><p className={`mt-1 text-sm font-semibold ${urgentOpenTickets.length ? "text-danger-700" : "text-ink-900"}`}>{urgentOpenTickets.length}</p></div>
          </div>
        </article>

        <article className="rounded-2xl border border-sand-200 bg-white p-5 shadow-premium-sm">
          <p className="text-xs font-semibold uppercase tracking-[0.14em] text-petroleum-700">Periodjämförelse</p>
          ...
          <div className="mt-5 divide-y divide-sand-100">
            {[
              ...
              <div key={label} className="py-4">
                ...
                <div className="mt-1 flex items-center justify-between gap-3 text-xs text-ink-500"><span>Föregående: {previous}</span><span>{delta}</span></div>
              </div>
            ))}
          </div>
        </article>
      ...
      <section className="grid gap-4 xl:grid-cols-[1.35fr_0.65fr]">
        <article className="overflow-hidden rounded-2xl border border-sand-200 bg-white shadow-premium-sm">
          <div className="flex flex-col gap-2 border-b border-sand-100 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
            <div><p className="text-xs font-semibold uppercase tracking-[0.14em] text-petroleum-700">Beståndsprestanda</p><h2 className="mt-1 font-display text-[19px] font-semibold text-ink-950">Fastigheter i fokus</h2></div>
            <p className="text-xs text-ink-500">{selectedProperty ? `Filtrerat på ${selectedProperty.name}` : `${propertyRows.length} fastigheter`}</p>
          </div>
          {propertyRows.length === 0 ? <p className="p-6 text-sm text-ink-500">Inga fastigheter att rapportera ännu.</p> : (
            <div className="overflow-x-auto">
              <table className="min-w-[860px] w-full text-left">
                <thead className="bg-surface-subtle text-xs uppercase tracking-[0.08em] text-ink-400"><tr><th className="px-5 py-3 font-semibold">Fastighet</th><th className="px-3 py-3 font-semibold">Uthyrning</th><th className="px-3 py-3 font-semibold">Ärenden</th><th className="px-3 py-3 font-semibold">Arbetsorder</th><th className="px-3 py-3 font-semibold">Hyresintäkt</th><th className="px-3 py-3 font-semibold">Kostnadsutfall</th><th className="px-5 py-3"><span className="sr-only">Öppna</span></th></tr></thead>
                <tbody className="divide-y divide-sand-100">
                  {propertyRows.map((property) => (
                    <tr key={property.id} className="transition hover:bg-sand-50/60">
                      <td className="px-5 py-4"><p className="text-xs font-semibold text-ink-900">{property.name}</p><p className="mt-0.5 text-xs text-ink-500">{property.city} · {property._count.units} objekt</p></td>
                      <td className="px-3 py-4"><div className="flex items-center gap-2"><div className="h-1.5 w-16 overflow-hidden rounded-full bg-sand-100"><div className="h-full rounded-full bg-petroleum-700" style={{ width: `${property.occupancy}%` }} /></div><span className="text-xs font-semibold text-ink-700">{property.occupancy} %</span></div></td>
                      ...
                      <td className="px-5 py-4 text-right"><Link href={`/dashboard/fastigheter/${property.id}`} className="inline-flex items-center gap-1 text-xs font-semibold text-petroleum-700 hover:text-petroleum-900">Öppna <ArrowRight className="h-3 w-3" /></Link></td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <article className="rounded-2xl border border-sand-200 bg-white p-5 shadow-premium-sm">
          <p className="text-xs font-semibold uppercase tracking-[0.14em] text-petroleum-700">Behöver uppmärksamhet</p>
      ...
      <section className="grid gap-4 lg:grid-cols-3">
        <article className="rounded-2xl border border-sand-200 bg-white p-5 shadow-premium-sm">
          <p className="text-xs font-semibold uppercase tracking-[0.14em] text-petroleum-700">Ärendebild</p>
        ...
        <article className="rounded-2xl border border-sand-200 bg-white p-5 shadow-premium-sm">
          <div className="flex items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[0.14em] text-petroleum-700">Ekonomi</p><h2 className="mt-1 font-display text-[18px] font-semibold text-ink-950">Årets registrerade utfall</h2></div><Link href="/dashboard/budget" className="print:hidden text-xs font-semibold text-petroleum-700 hover:text-petroleum-900">Budget</Link></div>
          <div className="mt-5 divide-y divide-sand-100">
            <div className="flex items-center justify-between py-3"><div><p className="text-xs font-medium text-ink-700">Intäktsutfall</p><p className="text-xs text-ink-500">Budget {compactMoney(budgetIncome)}</p></div><p className="text-sm font-semibold text-ink-950">{compactMoney(actualIncome)}</p></div>
            <div className="flex items-center justify-between py-3"><div><p className="text-xs font-medium text-ink-700">Kostnadsutfall</p><p className="text-xs text-ink-500">Budget {compactMoney(budgetCosts)}</p></div><p className="text-sm font-semibold text-ink-950">{compactMoney(actualCosts)}</p></div>
            <div className="flex items-center justify-between py-3"><div><p className="text-xs font-medium text-ink-700">Registrerat driftnetto</p><p className="text-xs text-ink-500">Budgeterat netto {compactMoney(budgetNet)}</p></div><p className={`text-sm font-semibold ${registeredNet < 0 ? "text-danger-700" : "text-ink-950"}`}>{compactMoney(registeredNet)}</p></div>
        ...
        <article className="rounded-2xl border border-sand-200 bg-white p-5 shadow-premium-sm">
          <p className="text-xs font-semibold uppercase tracking-[0.14em] text-petroleum-700">Förvaltningsaktivitet</p>
          ...
          <div className="mt-5 grid grid-cols-2 gap-2.5">
            {[
              ...
              return <div key={String(label)} className="rounded-xl border border-sand-100 bg-surface-subtle p-3"><ActivityIcon className="h-4 w-4 text-petroleum-700" /><p className="mt-3 text-lg font-semibold text-ink-950">{String(value)}</p><p className="mt-0.5 text-xs text-ink-500">{String(label)}</p></div>;
            })}
          </div>
        </article>
      </section>
      ...
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <div className="rounded-2xl border border-sand-200 bg-[#F4F2EC] p-4"><p className="text-xs text-ink-500">Samlad registrerad area</p><p className="mt-1.5 text-lg font-semibold text-ink-950">{Math.round(portfolioArea).toLocaleString("sv-SE")} m²</p></div>
        <div className="rounded-2xl border border-sand-200 bg-[#F4F2EC] p-4"><p className="text-xs text-ink-500">Byggnader i urvalet</p><p className="mt-1.5 text-lg font-semibold text-ink-950">{buildingCount}</p></div>
        <div className="rounded-2xl border border-sand-200 bg-[#F4F2EC] p-4"><p className="text-xs text-ink-500">Kontrakterad årshyra</p><p className="mt-1.5 text-lg font-semibold text-ink-950">{compactMoney(contractedAnnualRent)}</p></div>
        <div className="rounded-2xl border border-sand-200 bg-[#F4F2EC] p-4"><p className="text-xs text-ink-500">Uppsagda avtal</p><p className="mt-1.5 text-lg font-semibold text-ink-950">{noticeLeases}</p></div>
      </section>
    </div>
  );
}
