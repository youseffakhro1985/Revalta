...
import type { ResidentPortalDocumentsState } from "@/lib/resident-portal-documents";
...
type Props = {
  initial: ResidentPortalDocumentsState;
  selectedLeaseId: string;
  query: string;
};

export function ResidentDocuments({ initial, selectedLeaseId: initialLeaseId, query: initialQuery }: Props) {
  const [data, setData] = useState(initial);
  const [selectedLeaseId, setSelectedLeaseId] = useState(initialLeaseId);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setSelectedLeaseId((current) => {
        if (current && payload.leases?.some((lease: { id: string }) => lease.id === current)) return current;
      ...
      <Panel title="Välj boende och avtal" description="Dokumentlistan räknas om efter det valda aktiva avtalet.">
        {loading ? <div className="h-24 animate-pulse rounded-xl bg-sand-100" /> : !data.leases.length ? (
          ...
          <form method="get" action="/dashboard/boendeportal/dokument" className="grid gap-4 lg:grid-cols-[1fr_auto] lg:items-end">
            ...
            {selectedLease ? (
              <div className="rounded-xl border border-sand-200 bg-sand-50/70 px-4 py-3 lg:col-span-2">
