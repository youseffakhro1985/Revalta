      ...
      <aside className="fixed inset-y-0 left-0 z-40 hidden w-[248px] border-r border-sand-200 bg-[#F1F1EC] lg:flex lg:flex-col">
        <div className="flex h-[72px] items-center border-b border-sand-200 px-6">
          <Link href="/dashboard" className="flex items-center gap-3" aria-label="Revalta dashboard">
            ...
            <span className="text-[8px] font-semibold uppercase leading-[1.2] tracking-[0.13em] text-ink-400">
              Förvaltning
              <br />
              Sverige
            </span>
                ...
                {group.items.map((item) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {item.label}
                      {active ? <span className="ml-auto h-1.5 w-1.5 rounded-full bg-petroleum-600" /> : null}
                    </Link>
                  );
                })}
              </div>
        ...
        <div className="border-t border-sand-200 p-3">
          <div className="mb-1 flex items-center gap-3 rounded-lg px-3 py-2.5">
            <div className="flex h-8 w-8 items-center justify-center rounded-full bg-petroleum-100 text-[10px] font-semibold text-petroleum-800">RV</div>
            <div className="min-w-0">
              <p className="truncate text-[12px] font-semibold text-ink-800">Organisation</p>
              <p className="truncate text-[10px] text-ink-400">Aktiv användare</p>
            </div>
          </div>
