...
export default function RecurringWorkOrdersPage() {
  ...
  useEffect(() => {
    ...
    window.setTimeout(() => document.getElementById("schemalista-uppdatera")?.focus(), 0);
  }, [loading, schedules]);
  useEffect(() => {
    if (loading) return;
    if (window.location.hash !== "#korhistorik") return;
    document.getElementById("korhistorik")?.scrollIntoView({ behavior: "smooth", block: "start" });
    window.setTimeout(() => document.getElementById("korhistorik-kor")?.focus(), 0);
  }, [loading, runs]);
  ...
  return <div className="space-y-8">
    <PageHeader eyebrow="Förebyggande drift" title="Återkommande arbetsordrar" description="Skapa, övervaka och kör styrda scheman för tillsyn, service och förebyggande underhåll." action={<div className="flex flex-wrap gap-2"><Link href="/dashboard/arbetsorder/aterkommande/incidenter" className="inline-flex h-11 items-center gap-2 rounded-xl border border-sand-200 bg-white px-4 text-sm font-semibold text-ink-700"><AlertTriangle className="h-4 w-4" />Incidenter</Link><button id="korhistorik-kor" type="button" onClick={() => void runAll()} disabled={runningAll} className="inline-flex h-11 items-center gap-2 rounded-xl bg-petroleum-700 px-4 text-sm font-semibold text-white disabled:opacity-50"><PlayCircle className="h-4 w-4" />{runningAll ? "Kör…" : "Kör förfallna"}</button><button id="schemalista-uppdatera" type="button" onClick={() => void load()} className="inline-flex h-11 items-center gap-2 rounded-xl border border-sand-200 bg-white px-4 text-sm font-semibold text-ink-700"><RefreshCw className={`h-4 w-4 ${loading ? "animate-spin" : ""}`} /> Uppdatera</button></div>} />
    ...
    <section className="grid gap-6 xl:grid-cols-[390px_1fr]">
      ...
      <Panel title="Scheman" description={`${schedules.length} återkommande arbetsflöden`} bodyClassName="p-0">
        ...
        <div className="divide-y divide-sand-100">{schedules.map((item) => {
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${item.active ? "bg-petroleum-50 text-petroleum-700" : "bg-sand-100 text-ink-500"}`}>{item.active ? "Aktivt" : "Pausat"}</span><span className="text-xs font-semibold text-ink-500">{frequencyLabel[item.frequency]}</span></div><h3 className="mt-3 text-lg font-semibold text-ink-950">{item.title}</h3><p className="mt-1 text-sm text-ink-500">{item.property_name} · {priorityLabel[item.priority]} prioritet</p><p className="mt-3 line-clamp-2 text-sm leading-6 text-ink-600">{item.description}</p>{isLegacy ? <p className="mt-2 text-xs font-medium text-warning-800">Äldre schema – kör backfill till RecurringWorkOrderSchedule innan paus/generering.</p> : null}</div><div className="shrink-0 space-y-2 lg:text-right"><p className="text-xs font-semibold uppercase tracking-wide text-ink-500">Nästa körning</p><p className="mt-1 font-semibold text-ink-900">{dateTime.format(new Date(item.next_run_at))}</p><p className="mt-1 text-xs text-ink-500">{item.estimated_cost == null ? "Kostnad saknas" : currency.format(item.estimated_cost)}</p>{!isLegacy ? <button type="button" onClick={() => (editingId === item.id ? setEditingId("") : startEdit(item))} className="block text-xs font-semibold text-petroleum-800 transition hover:text-petroleum-950 lg:ml-auto">{editingId === item.id ? "Stäng" : "Ändra"}</button> : null}</div></div>
          ...
          {editingId === item.id && !isLegacy ? <div className="mt-4 space-y-3 border-t border-sand-100 pt-4"><input className={premiumFieldClass} placeholder="Rubrik" aria-label="Rubrik" value={editForm.title} onChange={(e) => setEditForm({ ...editForm, title: e.target.value })} /><textarea className={premiumTextareaClass} placeholder="Arbetsbeskrivning" aria-label="Arbetsbeskrivning" value={editForm.description} onChange={(e) => setEditForm({ ...editForm, description: e.target.value })} /><div className="grid grid-cols-2 gap-3"><select className={premiumFieldClass} aria-label="Frekvens" value={editForm.frequency} onChange={(e) => setEditForm({ ...editForm, frequency: e.target.value })}><option value="weekly">Vecka</option><option value="monthly">Månad</option><option value="quarterly">Kvartal</option><option value="yearly">År</option></select><select className={premiumFieldClass} aria-label="Prioritet" value={editForm.priority} onChange={(e) => setEditForm({ ...editForm, priority: e.target.value })}><option value="low">Låg</option><option value="normal">Normal</option><option value="high">Hög</option><option value="urgent">Akut</option></select></div><input className={premiumFieldClass} type="datetime-local" aria-label="Nästa körning" value={editForm.nextRunAt} onChange={(e) => setEditForm({ ...editForm, nextRunAt: e.target.value })} /><input className={premiumFieldClass} type="number" min="0" placeholder="Beräknad kostnad" aria-label="Beräknad kostnad" value={editForm.estimatedCost} onChange={(e) => setEditForm({ ...editForm, estimatedCost: e.target.value })} /><label className="flex items-center gap-2 text-sm text-ink-700"><input type="checkbox" checked={editForm.active} onChange={(e) => setEditForm({ ...editForm, active: e.target.checked })} className="h-4 w-4 rounded border-sand-300 text-petroleum-700" />Aktivt schema</label><button type="button" disabled={busyId === item.id} onClick={() => void saveEdit(item)} className={`${premiumPrimaryButtonClass} sm:w-auto`}>{busyId === item.id ? "Sparar…" : "Spara ändringar"}</button></div> : null}
        </article>;
        })}</div>
      </Panel>
      </div>
