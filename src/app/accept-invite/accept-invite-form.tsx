...
function inviteReasonError(reason: string, token: string) {
  if (reason === "invalid") return "Inbjudan är ogiltig eller har gått ut";
  if (reason === "policy") return passwordPolicyMessage;
  if (reason === "exists") return "Det finns redan ett konto med den här e-postadressen. Logga in i stället.";
  if (reason === "rate") return "För många försök. Vänta en stund och prova igen.";
  if (reason === "error") return "Något gick fel";
  if (!token) return "Inbjudningslänken saknas eller är ogiltig.";
  return "";
}

export function AcceptInviteForm({ token, reason }: { token: string; reason: string }) {
  const router = useRouter();
  const [preview, setPreview] = useState<InvitePreview | null>(null);
  const [message, setMessage] = useState("");
  const [error, setError] = useState(() => inviteReasonError(reason, token));
