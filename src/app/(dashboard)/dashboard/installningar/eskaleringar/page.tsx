...
export default function EscalationAdminPage() {
  ...
  useEffect(() => {
    ...
    window.setTimeout(() => document.getElementById("regellista-uppdatera")?.focus(), 0);
  }, [loading, data]);
  useEffect(() => {
    if (loading) return;
        ...
        <Panel title="Driftstatus" description="Kontroll av att eskaleringsmotorn har nödvändig produktionskonfiguration.">
          <div className="space-y-3">
            {[["CRON_SECRET", data?.configuration.cronSecret], ["EMAIL_PROVIDER_API_KEY", data?.configuration.emailApiKey], ["EMAIL_FROM", data?.configuration.emailFrom]].map(([label, active]) => (
              <div key={String(label)} className="flex items-center justify-between rounded-xl border border-sand-200 p-4"><span className="font-semibold text-ink-900">{String(label)}</span><span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-semibold ${active ? "bg-success-50 text-success-800" : "bg-danger-50 text-danger-700"}`}>{active ? <ShieldCheck className="h-3.5 w-3.5" /> : <AlertOctagon className="h-3.5 w-3.5" />}{active ? "Aktiv" : "Saknas"}</span></div>
            ))}
            <div className={`rounded-xl p-4 text-sm font-semibold ${configured ? "bg-success-50 text-success-800" : "bg-warning-50 text-warning-800"}`}>{configured ? "Eskaleringstjänsten är tekniskt redo." : "En eller flera produktionsvariabler behöver konfigureras."}</div>
          </div>
        </Panel>
          ...
          return (
            <details key={event.id} className="group rounded-xl border border-sand-200 bg-white">
              <summary className="grid cursor-pointer list-none gap-3 p-4 sm:grid-cols-[170px_120px_minmax(0,1fr)_auto] sm:items-center">
                ...
                <span className={`w-fit rounded-full px-2.5 py-1 text-xs font-semibold ${event.status === "sent" ? "bg-success-50 text-success-800" : event.status === "failed" ? "bg-danger-50 text-danger-700" : "bg-sand-100 text-ink-600"}`}>{statusLabels[event.status] || event.status}</span>
              ...
              <div className="border-t border-sand-100 p-5">
                ...
                {errorMessage ? <p className="mt-5 rounded-xl bg-danger-50 p-4 text-sm text-danger-700">{errorMessage}</p> : null}
