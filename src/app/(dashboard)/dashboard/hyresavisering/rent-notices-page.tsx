...
export function RentNoticesPage({ initialFocusedId }: { initialFocusedId: string }) {
  ...
  useEffect(() => {
    if (loading) return;
    const id = initialFocusedId || new URLSearchParams(window.location.search).get("id")?.trim() || "";
    if (!id) return;
    setFocusedId(id);
