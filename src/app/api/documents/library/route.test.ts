...
describe("documents/library GET — tenant and pagination contract", () => {
  ...
  it("scopes every document query to the session company and applies bounded server pagination", async () => {
    ...
    const response = await GET(new Request(
      "https://www.revalta.se/api/documents/library?page=3&pageSize=25&search=avtal&category=contract&visibility=internal&lifecycle=active&sort=name&focus=internal",
    ));
    ...
    for (const call of managedCountMock.mock.calls) {
      expect(call[0]?.where).toEqual(expect.objectContaining({ company_id: "company-a" }));
    }
    for (const call of managedGroupByMock.mock.calls) {
      expect(call[0]?.where).toEqual(expect.objectContaining({ company_id: "company-a" }));
    }
    ...
    expect(managedFindManyMock.mock.calls[0]?.[0]).toEqual(expect.objectContaining({
      skip: 50,
      take: 25,
