...
export function PreventiveMaintenanceOverview() {
  ...
  useEffect(() => {
    ...
    window.setTimeout(() => document.getElementById("service-alla")?.focus(), 0);
  }, [loading, data]);
  useEffect(() => {
    if (loading) return;
    ...
    window.setTimeout(() => document.getElementById("serviceoversikt-uppdatera")?.focus(), 0);
  }, [loading, data]);
    ...
    <div className="space-y-6">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        ...
        <div className="flex flex-wrap gap-2">
          <button id="serviceoversikt-uppdatera" type="button" onClick={() => void load()} className="inline-flex items-center gap-2 rounded-xl border border-sand-200 bg-white px-4 py-2 text-sm font-semibold text-ink-700 shadow-sm hover:bg-sand-50"><RefreshCw className="h-4 w-4" /> Uppdatera</button>
          ...
          <div className="divide-y divide-sand-100 overflow-hidden rounded-xl border border-sand-200 bg-white">
            {rows.map((row) => {
              ...
              return <div key={row.id} className="grid gap-4 p-4 lg:grid-cols-[minmax(0,1.5fr)_minmax(0,1fr)_minmax(0,1fr)_minmax(0,1fr)_auto] lg:items-center">
                ...
                <div className="text-sm text-ink-600"><span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-semibold ${state.className}`}>{state.label}</span><p className="mt-1 text-xs text-ink-500">Intervall {row.service_interval_months} mån · framförhållning {row.service_lead_days} dagar</p></div>
                <div className="text-sm"><p className="font-medium text-ink-800">{criticalityLabels[row.criticality || "normal"] || "Normal"} kritikalitet</p><p className="mt-1 text-xs text-ink-500">{row.auto_create_service_work_orders ? "Automatisk arbetsorder aktiv" : "Automatik avstängd"}</p></div>
                <div className="text-sm"><p className="font-medium text-ink-800">{row.maintenance_cycle_advanced_at ? `Cykel flyttad ${date.format(new Date(row.maintenance_cycle_advanced_at))}` : "Ingen avslutad cykel"}</p><p className="mt-1 text-xs text-ink-500">{row.completed_at ? `Service utförd ${date.format(new Date(row.completed_at))}` : "Inväntar genomförd service"}</p></div>
                <div className="flex justify-end">{row.work_order_id ? <Link href={`/dashboard/arbetsorder/${row.work_order_id}`} className="rounded-lg border border-sand-200 px-3 py-2 text-xs font-semibold text-petroleum-800 hover:bg-sand-50">{row.work_order_number || "Öppna arbetsorder"} · {statusLabels[row.work_order_status || ""] || row.work_order_status}</Link> : <span className="text-xs text-ink-500">Ingen arbetsorder skapad</span>}</div>
              </div>;
            })}
          </div>
        ) : null}
