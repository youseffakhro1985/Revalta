...
import { canFinalizeWorkOrderExecution, canMutateWorkOrderExecution, isWorkOrderExecutionLocked } from "@/lib/work-order-execution-policy";
...
export function WorkOrderExecutionPanel({ workOrderId }: Props) {
  ...
  return <div className="space-y-6" aria-busy={saving}>
    ...
    <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4" aria-label="Sammanfattning av arbetsorderutförande">
      <article className="rounded-2xl border border-sand-200 bg-white p-5 shadow-premium-sm"><div className="flex items-start justify-between"><div><p className="text-sm text-ink-500">Checklista</p><p className="mt-2 text-2xl font-semibold text-ink-950">{completed}/{checklist.length}</p><p className="mt-1 text-xs text-ink-500">{requiredIncomplete} obligatoriska återstår</p></div><CheckCircle2 className="h-5 w-5 text-petroleum-700" aria-hidden="true" /></div></article>
      ...
      <article className="rounded-2xl border border-sand-200 bg-white p-5 shadow-premium-sm"><div className="flex items-start justify-between"><div><p className="text-sm text-ink-500">Registrerat utfall</p><p className="mt-2 text-2xl font-semibold text-ink-950">{formattedFieldCost}</p>{!canViewFinance ? <p className="mt-1 text-xs text-ink-500">Kostnad kräver ekonomibehörighet</p> : null}</div><Banknote className="h-5 w-5 text-petroleum-700" aria-hidden="true" /></div></article>
      <article className="rounded-2xl border border-sand-200 bg-sand-50 p-5"><div className="flex items-start justify-between"><div><p className="text-sm text-ink-500">Bilddokumentation</p><p className="mt-2 text-2xl font-semibold text-ink-950">{completion.before_photo_count} / {completion.after_photo_count}</p><p className="mt-1 text-xs text-ink-500">Före / efter</p></div><Info className="h-5 w-5 text-petroleum-700" aria-hidden="true" /></div></article>
    </section>

    <Panel title="Slutkontroll" description={canViewFinance ? "Avslut sätter driftkostnad och skickar tid/material vidare till attestering under Ekonomi och fakturering." : "Avslut skickar registrerad tid och material vidare till behörig ekonomianvändare för attestering."}>
      <div className="grid gap-3 sm:grid-cols-3">
        <div className={`rounded-2xl border p-4 ${requiredIncomplete === 0 ? "border-petroleum-200 bg-petroleum-50 text-petroleum-800" : "border-warning-200 bg-warning-50 text-warning-800"}`}><p className="text-xs font-semibold uppercase tracking-wide">Kontrollpunkter</p><p className="mt-2 text-sm font-semibold">{requiredIncomplete === 0 ? "Alla obligatoriska klara" : `${requiredIncomplete} återstår`}</p></div>
      ...
      <p id="completion-help" className="mt-2 text-xs text-ink-500">{lifecycleLocked ? `${completionLabel}. Fältregistreringarna är låsta.` : !canManage ? "Du har läsbehörighet men kan inte ändra utförandet." : !canFinalizeWorkOrderExecution(completion.status) ? "Arbetsordern måste vara påbörjad innan den kan slutföras." : canFinalize ? "Vid avslut uppdateras driftkostnad och tid/material skapas som attesterbara rader. Finns en leverantör med kontaktadress i registret skickas ett avslutmejl." : "Slutför checklistan och ladda upp minst en efterbild i dokumentpanelen."}</p>
