...
export async function getLeaseHandoverReport(companyId: string, leaseId: string) {
  const lease = await db.lease.findFirst({
    ...
    select: {
      ...
      unit: { select: { designation: true, unit_type: true, area_sqm: true } },
  ...
  const handover = handoverEvent?.payload && typeof handoverEvent.payload === "object"
    ? handoverEvent.payload as unknown as LeaseHandoverPayload
    : null;
  const inspection = inspectionEvent?.payload && typeof inspectionEvent.payload === "object"
