...
type Data = {
  canManage: boolean; currentUserEmail: string;
  configuration: { cronSecret: boolean; emailApiKey: boolean; emailFrom: boolean; appUrl: string };
  preferences: Preferences; preferencesUpdatedAt: string | null;
  due: { total: number; overdue: number }; recipients: Recipient[]; events: EventRow[]; statusCounts: Record<string, number>;
};
...
export default function ServiceNotificationsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setData(body); setPreferences(body.preferences); setExtraEmails(body.preferences.additionalEmails.join("\n"));
    } catch (value) { setError(value instanceof Error ? value.message : "Kunde inte hämta aviseringsstatus"); }
  ...
  async function sendTest() {
    ...
    try {
      ...
      setSuccess(`Testutskicket skickades till ${body.recipient}.`); await load();
    } catch (value) { setError(value instanceof Error ? value.message : "Testutskicket misslyckades"); }
    finally { setSending(false); }
  }

  async function savePreferences(event: React.FormEvent) {
    event.preventDefault(); setSaving(true); setError(""); setSuccess("");
    const additionalEmails = extraEmails.split(/[\n,;]+/).map((email) => email.trim().toLowerCase()).filter(Boolean);
    try {
      ...
      setSuccess("Aviseringsinställningarna är sparade och används vid nästa dagliga körning."); await load();
    } catch (value) { setError(value instanceof Error ? value.message : "Kunde inte spara inställningarna"); }
      ...
      <form onSubmit={savePreferences}>
        <Panel title="Aviseringsinställningar" description="Inställningarna gäller endast den egna organisationen och versionsloggas vid varje ändring.">
          <fieldset disabled={!data?.canManage || saving} className="grid gap-6 disabled:opacity-60 lg:grid-cols-3">
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="mt-2 flex items-center gap-3"><input id="days-ahead" type="number" min={1} max={90} value={preferences.daysAhead} onChange={(event) => setPreferences((current) => ({ ...current, daysAhead: Number(event.target.value) }))} className="w-28 rounded-xl border border-sand-200 px-3 py-2.5" /><span className="text-sm text-ink-500">dagar</span></div>
                <p className="mt-2 text-xs text-ink-500">Tillåtet intervall: 1–90 dagar.</p>
              </div>
            </div>
            ...
            <div>
              ...
              <p className="mt-2 text-xs text-ink-500">Minst en roll måste vara vald.</p>
            </div>

            <div>
              <label htmlFor="extra-emails" className="block text-sm font-semibold text-ink-800">Extra e-postmottagare</label>
              <textarea id="extra-emails" rows={7} value={extraEmails} onChange={(event) => setExtraEmails(event.target.value)} placeholder="teknik@foretag.se\njour@foretag.se" className="mt-3 w-full rounded-xl border border-sand-200 px-3 py-3 text-sm" />
              <p className="mt-2 text-xs text-ink-500">En adress per rad, högst 20. Dubbletter tas bort automatiskt.</p>
            </div>
          </fieldset>
          <div className="mt-6 flex flex-col justify-between gap-3 border-t border-sand-100 pt-5 sm:flex-row sm:items-center">
            ...
            <button type="submit" disabled={!data?.canManage || saving || preferences.roles.length === 0} className="inline-flex items-center justify-center gap-2 rounded-xl bg-petroleum-800 px-5 py-2.5 text-sm font-semibold text-white hover:bg-petroleum-900 disabled:opacity-50"><Save className="h-4 w-4" /> {saving ? "Sparar…" : "Spara inställningar"}</button>
          </div>
        </Panel>
      </form>
