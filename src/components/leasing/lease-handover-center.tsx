...
export function LeaseHandoverCenter() {
  ...
  useEffect(() => {
    ...
    window.setTimeout(() => document.getElementById("overlamning-avtal")?.focus(), 0);
  }, [loading]);
  useEffect(() => {
    if (loading) return;
  ...
  useEffect(() => {
    ...
    window.setTimeout(() => document.getElementById("overlamninglista-uppdatera")?.focus(), 0);
  }, [loading, detail]);
