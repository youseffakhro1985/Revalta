...
import { PageHeader, premiumHeroSelectClass, premiumPrimaryButtonClass } from "@/components/dashboard/premium-ui";
...
function statusTone(status: string) {
  if (status === "completed" || status === "invoiced") return "bg-success-50 text-success-700 ring-success-100";
...
function priorityTone(priority: string) {
  ...
  if (priority === "low") return "bg-success-50 text-success-700 ring-success-100";
...
export default function WorkOrdersPage() {
  ...
  return <div className="space-y-4 sm:space-y-5">
    <PageHeader
      catalog="arbetsorder"
      eyebrow="Drift · Arbetsordrar"
      title="Arbetsordrar"
      description={scopedToAssigned
        ? "Planera, prioritera och följ arbetsordrar, SLA och fastigheter i en samlad arbetsyta. Vyn är begränsad till arbetsordrar som är tilldelade dig."
        : "Planera, prioritera och följ arbetsordrar, SLA, ansvariga och fastigheter i en samlad arbetsyta."}
      records={loading ? "Hämtar…" : String(orders.length)}
      status={scopedToAssigned ? "Begränsad vy" : "Live"}
      statusTone={scopedToAssigned ? "warn" : "ok"}
      action={(
        <div className="flex flex-wrap gap-2">
          <label className="relative">
            <CalendarDays className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-white/55" />
            <select value={range} onChange={(event) => setRange(event.target.value as RangeKey)} aria-label="Period" className={`${premiumHeroSelectClass} pl-9 pr-8`}>
              <option value="30">Senaste 30 dagarna</option>
              <option value="90">Senaste 90 dagarna</option>
              <option value="all">Alla perioder</option>
            </select>
          </label>
          {canManage ? <Link href="/dashboard/arbetsorder/ny" className={premiumPrimaryButtonClass}><Wrench className="mr-2 h-4 w-4" />Ny arbetsorder</Link> : null}
        </div>
      )}
    />
    ...
    <section className="grid gap-4 xl:grid-cols-[1.2fr_0.68fr_0.82fr]">
      <article className="rounded-2xl border border-sand-200 bg-white p-5 shadow-premium-sm">
        <div className="flex items-center justify-between gap-3"><div><h2 className="font-display text-[18px] font-semibold text-ink-950">Arbetsorderutveckling</h2><p className="mt-1 text-[13px] text-ink-450">Verkliga arbetsorder grupperade per vecka.</p></div><Link href="/dashboard/arbetsorder/operationsoversikt" className="rounded-lg border border-sand-200 bg-white px-3 py-2 text-[13px] font-semibold text-petroleum-700 hover:bg-petroleum-50">Veckovis</Link></div>
        <div className="mt-4 flex flex-wrap gap-4 text-[13px] text-ink-500"><Legend color="#29463f" text="Skapade" /><Legend color="#779e90" text="Planerade/pågående" /><Legend color="#c6b17b" text="Slutförda" /><Legend color="#dc2626" text="Försenade nu" /></div>
        <div className="mt-4 rounded-xl bg-surface-subtle px-3 py-4">
          ...
          <div className="grid grid-cols-8 text-center text-[12px] text-ink-400">{weekly.map((point) => <span key={point.label}>{point.label}</span>)}</div>
        </div>
        <Link href="/dashboard/arbetsorder/operationsoversikt" className="mt-4 inline-flex items-center gap-1 text-[13px] font-semibold text-petroleum-700 hover:text-petroleum-900">Visa detaljerad rapport <ArrowRight className="h-3 w-3" /></Link>
      </article>

      <article className="rounded-2xl border border-sand-200 bg-white p-5 shadow-premium-sm">
        <div className="flex items-center justify-between gap-3"><h2 className="font-display text-[18px] font-semibold text-ink-950">Statusfördelning</h2><button type="button" onClick={() => setFocus("all")} className="rounded-lg border border-sand-200 bg-white px-3 py-2 text-[13px] font-semibold text-ink-600 hover:bg-sand-50">Alla arbetsorder</button></div>
        <div className="mt-5 divide-y divide-sand-100">{statusRows.map((row) => <div key={row.status} className="grid grid-cols-[minmax(0,1fr)_72px_42px] items-center gap-3 py-3"><div className="min-w-0"><div className="flex items-center gap-2"><span className={`h-2 w-2 rounded-full ${row.status === "completed" ? "bg-success-500" : row.status === "in_progress" ? "bg-warning-500" : row.status === "waiting_material" || row.status === "blocked" ? "bg-violet-500" : "bg-sky-500"}`} /><span className="truncate text-[13px] font-semibold text-ink-700">{statusLabels[row.status] || row.status}</span></div><div className="mt-2 h-1.5 overflow-hidden rounded-full bg-sand-100"><div className="h-full rounded-full bg-petroleum-700" style={{ width: `${Math.max(row.count ? 8 : 0, row.percent)}%` }} /></div></div><span className="text-right text-[13px] font-semibold text-ink-700">{row.count}</span><span className="text-right text-[12px] text-ink-400">{row.percent}%</span></div>)}</div>
        <button type="button" onClick={() => setFocus("all")} className="mt-4 inline-flex items-center gap-1 text-[13px] font-semibold text-petroleum-700 hover:text-petroleum-900">Visa alla statusar <ArrowRight className="h-3 w-3" /></button>
      </article>

      <article className="overflow-hidden rounded-2xl border border-sand-200 bg-white shadow-premium-sm">
        <div className="flex items-center justify-between px-4 py-3.5"><div><h2 className="font-display text-[17px] font-semibold text-ink-950">Arbetsorder på karta</h2><p className="mt-0.5 text-[12px] text-ink-450">Klicka mellan arbetsorder och planera rutt.</p></div><MapPin className="h-4 w-4 text-petroleum-700" /></div>
        <div className="flex gap-1.5 border-y border-sand-100 px-3 py-2"><MapChip label="Akut" dot="bg-danger-500" onClick={() => setFocus("urgent")} /><MapChip label="Pågående" dot="bg-warning-500" onClick={() => setFocus("active")} /><MapChip label="Klar" dot="bg-success-600" onClick={() => setFocus("completed")} /></div>
        {selectedMapOrder ? <>
          <div className="relative h-[225px] bg-sand-100">
            ...
            <div className="pointer-events-none absolute left-3 top-3 max-w-[220px] rounded-xl border border-white/80 bg-white/95 px-3 py-2 shadow-premium-sm backdrop-blur-sm"><p className="text-[13px] font-semibold text-ink-900">{workOrderNumber(selectedMapOrder)}</p><p className="mt-0.5 truncate text-[12px] text-ink-500">{selectedMapOrder.property.name} · {selectedMapOrder.property.address}</p><p className="mt-1 text-[12px] font-semibold text-petroleum-700">{priorityLabels[selectedMapOrder.priority] || selectedMapOrder.priority} · {selectedMapOrder.title}</p></div>
          </div>
          <div className="border-t border-sand-100 p-3">
            {mapOrders.length > 1 ? <select value={selectedMapOrder.id} onChange={(event) => setSelectedMapId(event.target.value)} aria-label="Välj arbetsorder på kartan" className="mb-2.5 h-9 w-full rounded-xl border border-sand-200 bg-surface-subtle px-3 text-[13px] font-semibold text-ink-650 outline-none focus:ring-2 focus:ring-petroleum-100">{mapOrders.map((order) => <option key={order.id} value={order.id}>{workOrderNumber(order)} · {order.property.name}</option>)}</select> : null}
            <div className="grid grid-cols-2 gap-2"><a href={mapExternal} target="_blank" rel="noreferrer" className="inline-flex h-9 items-center justify-center gap-1.5 rounded-xl border border-sand-200 bg-white px-2 text-[12px] font-semibold text-petroleum-700 hover:bg-petroleum-50">Öppna karta <ExternalLink className="h-3 w-3" /></a><a href={routeUrl(mapOrders)} target="_blank" rel="noreferrer" className="inline-flex h-9 items-center justify-center gap-1.5 rounded-xl bg-petroleum-900 px-2 text-[12px] font-semibold text-white hover:bg-petroleum-800"><Route className="h-3.5 w-3.5" />Planera rutt</a></div>
          </div>
        </> : <div className="flex h-[290px] flex-col items-center justify-center px-6 text-center"><MapPin className="h-7 w-7 text-sand-400" /><p className="mt-2 text-[11px] font-semibold text-ink-700">Ingen arbetsorder med plats i filtret.</p><button type="button" onClick={() => { setFocus("all"); setQuery(""); }} className="mt-3 text-[13px] font-semibold text-petroleum-700">Rensa filter</button></div>}
      </article>
    </section>

    <section className="grid gap-4 xl:grid-cols-[1.24fr_0.78fr_0.86fr]">
      <article className="overflow-hidden rounded-2xl border border-sand-200 bg-white shadow-premium-sm">
        <SectionHead title="Senaste arbetsorder" action={<button type="button" onClick={() => { setFocus("all"); setQuery(""); }} className="inline-flex items-center gap-1 text-[13px] font-semibold text-petroleum-700">Visa alla <ArrowRight className="h-3 w-3" /></button>} />
        {recent.length ? <div className="overflow-x-auto"><table className="w-full min-w-[700px] text-left"><thead><tr className="bg-surface-subtle text-[12px] text-ink-400"><th className="px-4 py-2.5">Ärendenr</th><th className="px-3">Fastighet</th><th className="px-3">Kategori</th><th className="px-3">Prioritet</th><th className="px-3">Status</th><th className="px-3">Ansvarig</th><th className="px-3">Datum</th><th className="px-3" /></tr></thead><tbody className="divide-y divide-sand-100">{recent.map((order) => <tr key={order.id} className="text-[12px] text-ink-600 transition hover:bg-petroleum-50/40"><td className="px-4 py-3 font-semibold text-ink-850"><Link href={`/dashboard/arbetsorder/${order.id}`}>{workOrderNumber(order)}</Link></td><td className="px-3"><Link href={`/dashboard/fastigheter/${order.property.id}`} className="hover:text-petroleum-800">{order.property.name}</Link></td><td className="px-3">{typeLabels[order.enterprise?.work_type || ""] || "Arbetsorder"}</td><td className="px-3"><span className={`rounded-full px-2 py-1 font-semibold ring-1 ${priorityTone(order.priority)}`}>{priorityLabels[order.priority] || order.priority}</span></td><td className="px-3"><span className={`rounded-full px-2 py-1 font-semibold ring-1 ${statusTone(order.status)}`}>{statusLabels[order.status] || order.status}</span></td><td className="max-w-[120px] truncate px-3">{order.assigned_to?.name || order.assigned_to?.email || "Ej tilldelad"}</td><td className="px-3">{dateFmt.format(new Date(order.created_at))}</td><td className="px-3"><Link href={`/dashboard/arbetsorder/${order.id}`} aria-label={`Öppna ${workOrderNumber(order)}`}><ArrowRight className="h-3.5 w-3.5 text-petroleum-700" /></Link></td></tr>)}</tbody></table></div> : <Empty title={loading ? "Läser arbetsordrar…" : "Inga arbetsordrar i filtret"} />}
        <div className="border-t border-sand-100 px-4 py-3"><button type="button" onClick={() => { setFocus("all"); setQuery(""); }} className="inline-flex items-center gap-1 text-[13px] font-semibold text-petroleum-700">Visa alla arbetsorder <ArrowRight className="h-3 w-3" /></button></div>
      </article>

      <article className="rounded-2xl border border-sand-200 bg-white p-5 shadow-premium-sm">
        <h2 className="font-display text-[18px] font-semibold text-ink-950">Fördelning per kategori</h2>
        <div className="mt-5 flex items-center gap-5"><div className="relative h-36 w-36 shrink-0 rounded-full" style={{ background: donut }}><div className="absolute inset-[24px] flex flex-col items-center justify-center rounded-full bg-white"><span className="font-display text-2xl font-semibold text-ink-950">{orders.filter((order) => order.status !== "cancelled").length}</span><span className="text-[12px] text-ink-450">Totalt</span></div></div><div className="min-w-0 flex-1 space-y-2.5">{categories.map(([category, count], index) => <div key={category} className="flex items-center gap-2 text-[12px]"><span className="h-2 w-2 rounded-full" style={{ backgroundColor: categoryColors[index % categoryColors.length] }} /><span className="min-w-0 flex-1 truncate text-ink-600">{typeLabels[category] || category}</span><span className="font-semibold text-ink-750">{Math.round((count / categoryTotal) * 100)}%</span></div>)}</div></div>
        <Link href="/dashboard/arbetsorder/operationsoversikt" className="mt-5 inline-flex items-center gap-1 text-[13px] font-semibold text-petroleum-700">Visa detaljer <ArrowRight className="h-3 w-3" /></Link>
      </article>

      <article className="rounded-2xl border border-sand-200 bg-white shadow-premium-sm">
        <SectionHead title="Dagens planering" action={<Link href="/dashboard/arbetsorder/planering" className="inline-flex items-center gap-1 text-[13px] font-semibold text-petroleum-700">Visa alla <ArrowRight className="h-3 w-3" /></Link>} />
        {todayPlanning.length ? <div className="divide-y divide-sand-100 px-4">{todayPlanning.map((order) => <Link key={order.id} href={`/dashboard/arbetsorder/${order.id}`} className="grid grid-cols-[44px_32px_minmax(0,1fr)_auto] items-center gap-2 py-3 transition hover:bg-petroleum-50/40"><span className="text-[12px] font-semibold text-ink-500">{timeFmt.format(new Date(order.scheduled_start || ""))}</span><span className="flex h-8 w-8 items-center justify-center rounded-full bg-petroleum-50 text-[12px] font-semibold text-petroleum-800">{(order.assigned_to?.name || order.assigned_to?.email || "ET").split(/\s|@/).slice(0, 2).map((part) => part[0]).join("").toUpperCase()}</span><span className="min-w-0"><span className="block truncate text-[13px] font-semibold text-ink-850">{order.assigned_to?.name || order.assigned_to?.email || "Ej tilldelad"}</span><span className="mt-0.5 block truncate text-[12px] text-ink-450">{order.property.name} · {order.title}</span></span><ArrowRight className="h-3.5 w-3.5 text-petroleum-700" /></Link>)}</div> : <Empty title={loading ? "Läser dagens planering…" : "Inga arbetsorder schemalagda idag"} />}
        <div className="border-t border-sand-100 px-4 py-3"><Link href="/dashboard/arbetsorder/planering" className="inline-flex items-center gap-1 text-[13px] font-semibold text-petroleum-700">Se hela dagens schema <ArrowRight className="h-3 w-3" /></Link></div>
      </article>
    </section>
  </div>;
}
...
function MapChip({ label, dot, onClick }: { label: string; dot: string; onClick: () => void }) {
  return <button type="button" onClick={onClick} className="inline-flex items-center gap-1.5 rounded-full border border-sand-200 bg-white px-2.5 py-1 text-[12px] font-semibold text-ink-650 hover:bg-sand-50"><span className={`h-2 w-2 rounded-full ${dot}`} />{label}</button>;
}
