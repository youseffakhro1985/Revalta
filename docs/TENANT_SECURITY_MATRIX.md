...
| Search / export | Company where clauses | CSV cells quoted; maintenance-plan, audit and SLA-report exports prefix `=`/`+`/`-`/`@` | Search company scope; ticket export company scope; ticket export quotes `=`/`+` cells; maintenance-plan export Tenant B property 404; maintenance-plan/audit/SLA-report CSV prefixes Tenant B formula titles | Remaining client-side CSV surfaces |
