...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const links = await db.integrationEvent.findMany({
    where: { company_id: user.company_id, type: LINK_EVENT, recipient: { startsWith: `${id}:` } },
    orderBy: { created_at: "desc" },
    take: 200,
  });
  const payloads = links.map((link) => link.payload as unknown as LinkPayload).filter((item) => item?.workOrderId);
  const ids = [...new Set(payloads.map((item) => item.workOrderId))];
  const workOrders = ids.length ? await db.workOrder.findMany({
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!body || Number(body.version) !== record.version) return NextResponse.json({ error: "Besiktningen har ändrats. Ladda om och försök igen." }, { status: 409 });
    const requestedIds = Array.isArray(body.itemIds) ? [...new Set(body.itemIds.filter((value): value is string => typeof value === "string"))] : [];
    ...
    const existingLinks = await db.integrationEvent.findMany({
      where: { company_id: user.company_id, type: LINK_EVENT, recipient: { in: items.map((item) => `${id}:${item.id}`) } },
      orderBy: { created_at: "desc" },
    });
    const existingItemIds = new Set(existingLinks.map((link) => (link.payload as unknown as LinkPayload)?.itemId).filter(Boolean));
    ...
    const created = await db.$transaction(async (tx) => {
      ...
      for (const item of pending) {
        ...
        const workOrder = await tx.workOrder.create({ data: {
          company_id: user.company_id!, property_id: lease.property_id, unit_id: lease.unit_id, created_by_id: user.id,
          title: `${item.area} · ${item.component} · ${lease.property.name}`,
          description: [`Besiktningsåtgärd för avtal ${lease.lease_number}.`, `Hyrespart: ${lease.lease_holder.name}`, `Anmärkning: ${item.description}`, item.recommendation ? `Rekommenderad åtgärd: ${item.recommendation}` : ""].filter(Boolean).join("\n\n"),
          status: "planned", priority, created_at: createdAt,
        } });
        await setWorkOrderEnterpriseFields(tx, { workOrderId: workOrder.id, companyId: user.company_id!, workOrderNumber, workType: "corrective", source: "inspection", responseDueAt: sla.responseDueAt, resolutionDueAt: sla.resolutionDueAt });
        await setWorkOrderAssetLinks(tx, { workOrderId: workOrder.id, companyId: user.company_id!, buildingId: null, technicalAssetId: null });
        await addWorkOrderStatusEvent(tx, { companyId: user.company_id!, workOrderId: workOrder.id, actorUserId: user.id, fromStatus: null, toStatus: "planned", reason: "Skapad från besiktningspunkt", metadata: { leaseId: id, itemId: item.id, area: item.area, component: item.component, recordVersion: record.version } });
        await tx.integrationEvent.create({ data: { company_id: user.company_id!, type: LINK_EVENT, recipient: `${id}:${item.id}`, status: "linked", payload: { leaseId: id, itemId: item.id, recordVersion: record.version, workOrderId: workOrder.id, createdAt: createdAt.toISOString() } } });
        result.push({ itemId: item.id, workOrderId: workOrder.id, workOrderNumber });
      }
      await tx.auditLog.create({ data: { company_id: user.company_id!, actor_user_id: user.id, entity_type: "lease_inspection", entity_id: id, action: "lease_inspection.work_orders_created", metadata: { leaseNumber: lease.lease_number, recordVersion: record.version, count: result.length, itemIds: result.map((item) => item.itemId), workOrderIds: result.map((item) => item.workOrderId) } } });
      return result;
    });
