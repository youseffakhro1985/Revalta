...
export async function PATCH(
  ...
  const existing = await db.workOrder.findFirst({
    ...
    select: { id: true, status: true, property_id: true, assigned_to_id: true },
  });
  ...
  if (body.title !== undefined) {
    ...
    if (!title) return NextResponse.json({ error: "Rubrik får inte vara tom" }, { status: 400 });
    data.title = title;
  }
  if (body.description !== undefined) {
    const description = String(body.description).trim();
    if (!description) return NextResponse.json({ error: "Beskrivning får inte vara tom" }, { status: 400 });
    data.description = description;
  }
  if (body.status !== undefined) {
    const status = String(body.status);
    if (!allowedStatuses.has(status)) return NextResponse.json({ error: "Ogiltig status" }, { status: 400 });
    data.status = status;
    data.completed_at = status === "completed" ? new Date() : null;
  }
  if (body.priority !== undefined) {
    const priority = String(body.priority);
    if (!allowedPriorities.has(priority)) return NextResponse.json({ error: "Ogiltig prioritet" }, { status: 400 });
  ...
  if (data.scheduled_start && data.scheduled_end && data.scheduled_end < data.scheduled_start) {
