...
function statusClass(status: string) {
  ...
  if (status === "reopened") return "bg-warning-50 text-warning-700";
  ...
  if (status.startsWith("level_")) return "bg-danger-50 text-danger-700";
...
function slaClass(status: string) {
  if (status === "met") return "bg-success-50 text-success-700";
  if (status === "breached") return "bg-danger-50 text-danger-700";
  if (status === "at_risk") return "bg-warning-50 text-warning-700";
...
export default function RecurringIncidentsPage() {
  ...
  useEffect(() => {
    ...
    window.setTimeout(() => document.getElementById("kontrollera-eskalering-knapp")?.focus(), 0);
  }, [loading]);
  useEffect(() => {
    if (loading) return;
    ...
    window.setTimeout(() => document.getElementById("incidentlista-uppdatera")?.focus(), 0);
  }, [loading, alerts]);
  ...
  return <div className="space-y-8">
    <PageHeader eyebrow="Driftincidenter" title="Incidenter för återkommande arbetsordrar" description="Tilldela ansvar, styr SLA, kvittera och följ upp misslyckade körningar och kraftigt försenade scheman." action={<div className="flex flex-wrap gap-2"><Link href="/dashboard/arbetsorder/aterkommande" className="inline-flex h-11 items-center gap-2 rounded-xl border border-sand-200 bg-white px-4 text-sm font-semibold text-ink-700"><ChevronLeft className="h-4 w-4" /> Till scheman</Link><Link href="/dashboard/arbetsorder/aterkommande/incidenter/sla-rapport" className="inline-flex h-11 items-center gap-2 rounded-xl border border-sand-200 bg-white px-4 text-sm font-semibold text-ink-700">SLA-rapport</Link><span id="kontrollera-eskalering" className="scroll-mt-36"><button id="kontrollera-eskalering-knapp" type="button" autoFocus onClick={() => void runEscalation()} disabled={runningEscalation} className="inline-flex h-11 items-center gap-2 rounded-xl bg-danger-700 px-4 text-sm font-semibold text-white disabled:opacity-50"><Siren className="h-4 w-4" /> {runningEscalation ? "Kontrollerar…" : "Kontrollera eskalering"}</button></span><button id="incidentlista-uppdatera" type="button" onClick={() => void load()} className="inline-flex h-11 items-center gap-2 rounded-xl border border-sand-200 bg-white px-4 text-sm font-semibold text-ink-700"><RefreshCw className={`h-4 w-4 ${loading ? "animate-spin" : ""}`} /> Uppdatera</button></div>} />
