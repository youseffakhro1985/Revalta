...
export default function PropertiesPage() {
  ...
  return (
    <div className="space-y-4 sm:space-y-5">
      ...
      <section className="rounded-2xl border border-sand-200 bg-white shadow-premium-sm" aria-label="Nyckeltal för fastigheter">
        <dl className="grid divide-y divide-sand-100 sm:grid-cols-2 sm:divide-y-0 xl:grid-cols-4 xl:divide-x">
          <KpiCell icon={Building2} label={hasFilters ? "Matchande fastigheter" : "Fastigheter"} value={pagination.total.toLocaleString("sv-SE")} helper={hasFilters ? "Efter aktuell sökning och filter" : "Aktivt bestånd i er organisation"} />
          <KpiCell icon={Layers3} label="Objekt på sidan" value={visibleTotals.units.toLocaleString("sv-SE")} helper={`Summerat över ${properties.length} visade fastigheter`} />
          <KpiCell icon={Wrench} label="Aktiva ärenden på sidan" value={visibleTotals.tickets.toLocaleString("sv-SE")} helper="Öppna poster på visade fastigheter" />
          <KpiCell icon={MapPin} label="Byggnader på sidan" value={visibleTotals.buildings.toLocaleString("sv-SE")} helper="Registrerade byggnader i aktuell sida" />
        </dl>
      </section>
      ...
      <section className="grid gap-4 2xl:grid-cols-[minmax(0,1fr)_290px]">
        <div className="overflow-hidden rounded-2xl border border-sand-200 bg-white shadow-premium-sm">
          <div className="flex items-center justify-between gap-4 border-b border-sand-100 px-5 py-4">
            <div>
              <h2 className="text-[15px] font-semibold tracking-[-0.01em] text-ink-950">Fastigheter <span className="font-normal text-ink-450">({pagination.total.toLocaleString("sv-SE")})</span></h2>
              <p className="mt-0.5 text-[11px] text-ink-500">Klicka på en rad för att öppna fastighetskortet.</p>
            </div>
        ...
        <aside className="space-y-3">
          ...
          <SidePanel title="Om listan" icon={Layers3}>
            <div className="space-y-2 text-[11px] leading-5 text-ink-500">
              <p>Listan visar {pageSize} fastigheter åt gången. Sökning och filter görs på hela beståndet, inte bara på sidan du ser.</p>
              <p>Export skapar en CSV-fil av hela det filtrerade resultatet.</p>
            </div>
          </SidePanel>
...
function KpiCell({ icon: Icon, label, value, helper }: { icon: LucideIcon; label: string; value: string; helper: string }) {
  return (
    <div className="flex items-start justify-between gap-3 px-5 py-4">
      <div className="min-w-0">
        ...
        <p className="mt-1.5 text-[10px] leading-4 text-ink-400">{helper}</p>
      </div>
      <span className="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-petroleum-50 text-petroleum-700"><Icon className="h-4 w-4" /></span>
    </div>
  );
}
