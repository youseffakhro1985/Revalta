...
export function WorkOrderSlaDetailPanel({ workOrderId }: Props) {
  ...
  useEffect(() => {
    ...
    window.setTimeout(() => document.getElementById("sla-svar-senast")?.focus(), 0);
  }, [loading, sla]);
  useEffect(() => {
    if (loading) return;
