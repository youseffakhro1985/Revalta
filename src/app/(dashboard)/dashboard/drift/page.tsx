  ...
  return (
    <div className="mx-auto max-w-6xl animate-fade-in space-y-8">
      <header className="rounded-2xl border border-sand-200 bg-white p-8 shadow-card">
        <p className="mb-3 text-sm font-semibold uppercase tracking-[0.2em] text-petroleum-600">Drift</p>
        <h1 className="text-4xl font-semibold tracking-tight text-ink-950">Systemhälsa och produktion</h1>
      ...
      {health ? (
        <>
          ...
          <section className="rounded-2xl border border-sand-200 bg-white p-6 shadow-card">
            <h2 className="text-xl font-bold text-ink-950">Miljö och integrationer</h2>
            <div className="mt-5 grid grid-cols-1 gap-3 md:grid-cols-3">
              {Object.entries(health.env).map(([key, value]) => (
                <div key={key} className="rounded-2xl border border-sand-100 bg-sand-50 p-4">
                  <p className="font-bold text-ink-950">{labels[key] || key}</p>
