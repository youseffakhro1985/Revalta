...
export default function EconomyPage(){
  ...
  return <div className="space-y-4 sm:space-y-5">
    <PageHeader catalog="ekonomi" eyebrow="Ekonomi & analys" title="Ekonomi" description="Hyresintäkter, budget, utfall och betalningsläge i en samlad ekonomisk arbetsyta med live-koppling till avier och budgetkonton." records={loading ? "Hämtar…" : `${es.length} konton · ${ns.length} avier`} action={<div className="flex flex-wrap gap-2"><select value={range} onChange={e=>setRange(e.target.value as RangeKey)} aria-label="Period" className={premiumHeroSelectClass}><option value="quarter">Senaste kvartalet</option><option value="year">Detta år</option><option value="all">Alla perioder</option></select>{canManage?<Link href="/dashboard/ekonomi/ny-utbetalning" className={premiumPrimaryButtonClass}><CircleDollarSign className="mr-2 h-4 w-4"/>Ny utbetalning</Link>:null}</div>} />
