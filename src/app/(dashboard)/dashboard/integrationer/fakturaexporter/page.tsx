  ...
  return <div className="mx-auto max-w-7xl space-y-6 animate-fade-in-soft">
    ...
    <Panel title="Filter och sökning" description="Filtrera på leverantör, status, arbetsorder, fastighet eller externt faktura-ID.">
      <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_220px_220px]">
        <label className="relative block"><Search className="pointer-events-none absolute left-3 top-3 h-4 w-4 text-ink-400" /><input value={query} onChange={event => setQuery(event.target.value)} placeholder="Sök arbetsorder, fastighet, jobb-ID eller fel..." className="w-full rounded-xl border border-sand-200 bg-white py-2.5 pl-10 pr-3 text-sm outline-none focus:border-petroleum-500" /></label>
        <select value={provider} onChange={event => setProvider(event.target.value)} className="rounded-xl border border-sand-200 bg-white px-3 py-2.5 text-sm"><option value="">Alla leverantörer</option>{data?.providers.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select>
        <select value={status} onChange={event => setStatus(event.target.value)} className="rounded-xl border border-sand-200 bg-white px-3 py-2.5 text-sm"><option value="">Alla statusar</option>{Object.entries(statusLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select>
      </div>
    </Panel>
