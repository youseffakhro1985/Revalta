...
| Quotes / vendors / insurance | Company filters present | Vendor create property via `findCompanyOwned`; quote POST re-reads property via `tenantWhere`; quote PATCH/work-order look up quote by session `company_id` | Vendor POST Tenant B propertyId 404; quote POST Tenant B propertyId 404; PATCH Tenant B quote id 404; work-order POST Tenant B quote id 404 | Remaining quote document/output surfaces |
| Energy / IMD / budget / calendar | Company filters present | Property and IMD lease re-read in caller company | Energy POST Tenant B propertyId 404; DELETE Tenant B reading id 404; IMD POST Tenant B propertyId 404; IMD POST Tenant B leaseId 404 | Calendar/budget remaining related-id tests |
