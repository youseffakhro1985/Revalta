...
6. **Continued hardening (P0/P1)**
   ...
   - Fail-closed lease inspection/handover mutations: `PUT` inspection-items and handover upsert modern tables only; IE-only leases return Swedish `409` asking for backfill (`LeaseInspectionRecord` / `LeaseHandoverRecord`) and do not rematerialize IE payloads. First create still allowed when neither modern nor IE product state exists. `createInspectionWorkOrders` (and inspection-items work-orders `POST`) require modern `LeaseInspectionRecord`; reconcile already fail-closed. GET dual-read unchanged.
