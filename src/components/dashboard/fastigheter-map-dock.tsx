      ...
      <div className="pointer-events-auto overflow-hidden rounded-2xl border border-sand-200 bg-white shadow-premium-lg">
        ...
        >
          ...
          <span className="min-w-0 flex-1">
            <span className="block text-xs font-semibold text-ink-900">Fastighetskarta</span>
            <span className="mt-0.5 block truncate text-[11px] text-ink-450">
        ...
        {open ? (
          <div className="border-t border-sand-100">
            {error ? (
              <div className="px-4 py-4 text-sm text-danger-700">{error}</div>
            ) : !selected ? (
              <div className="px-4 py-5 text-center">
                <Building2 className="mx-auto h-6 w-6 text-sand-400" />
                <p className="mt-2 text-xs font-semibold text-ink-750">Kartan visas när första fastigheten är registrerad.</p>
              </div>
            ) : (
              <>
                <div className="relative h-[156px] overflow-hidden bg-sand-100">
                  ...
                  <div className="pointer-events-none absolute left-3 top-3 rounded-lg border border-white/70 bg-white/92 px-2.5 py-1.5 shadow-sm backdrop-blur-sm">
                    <p className="max-w-[230px] truncate text-xs font-semibold text-ink-850">{selected.name}</p>
                  </div>
                </div>
                ...
                <div className="space-y-3 p-3.5">
                  {properties.length > 1 ? (
                    <label className="block">
                      ...
                      <select
                        ...
                        className="h-9 w-full rounded-xl border border-sand-200 bg-surface-subtle px-3 text-xs font-semibold text-ink-700 outline-none focus:border-petroleum-300 focus:ring-2 focus:ring-petroleum-100"
                      >
                  ...
                  <div className="flex items-start gap-2.5">
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="truncate text-xs font-semibold text-ink-850">{selected.name}</p>
                      <p className="mt-0.5 line-clamp-2 text-[11px] leading-4 text-ink-450">{address}</p>
                    </div>
                    <span className="shrink-0 rounded-full border border-sand-200 bg-sand-50 px-2 py-1 text-[11px] font-semibold text-ink-550">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <Link
                      ...
                      className="inline-flex h-9 items-center justify-center gap-1.5 rounded-xl bg-petroleum-900 px-3 text-xs font-semibold text-white transition hover:bg-petroleum-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-petroleum-300"
                    >
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="inline-flex h-9 items-center justify-center gap-1.5 rounded-xl border border-sand-200 bg-white px-3 text-xs font-semibold text-petroleum-750 transition hover:bg-petroleum-50"
                    >
