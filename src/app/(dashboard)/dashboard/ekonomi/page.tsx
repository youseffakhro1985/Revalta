...
import { AttestationQueuePanel } from "@/components/dashboard/attestation-queue-panel";
import { InvoiceBasisQueuePanel } from "@/components/dashboard/invoice-basis-queue-panel";
import { InvoiceCloseQueuePanel } from "@/components/dashboard/invoice-close-queue-panel";
import { InvoiceExportQueuePanel } from "@/components/dashboard/invoice-export-queue-panel";
import { RentNoticeStatusQueuePanel } from "@/components/dashboard/rent-notice-status-queue-panel";
...
const statusText:Record<string,string> = { draft:"Utkast", sent:"Skickad", paid:"Markerad betald", overdue:"Förfallen", credited:"Krediterad" };
...
export default function EconomyPage(){
  ...
  return <div className="space-y-4 sm:space-y-5">
    ...
    <InvoiceExportQueuePanel />
    <InvoiceCloseQueuePanel />
    <RentNoticeStatusQueuePanel />
    <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><Kpi icon={CircleDollarSign} label="Hyresintäkter" value={loading?"—":compact(m.income)} helper={`${ns.length} avier i vald period`} href="/dashboard/hyresavisering"/><Kpi icon={LineChart} label="Driftnetto" value={loading?"—":compact(m.net)} helper={`${compact(m.operations+m.finance)} kostnader`} href="/dashboard/budget" negative={m.net<0}/><Kpi icon={FileText} label="Förfallna avier" value={loading?"—":compact(m.overdue)} helper="Manuell aviestatus" href="/dashboard/hyresavisering" negative={m.overdue>0}/><Kpi icon={Banknote} label="Kassaflöde" value={loading?"—":compact(m.cash)} helper={`${compact(m.investments)} investeringar`} href="/dashboard/rapporter" negative={m.cash<0}/></section>
    <section className="grid gap-4 xl:grid-cols-[1.35fr_0.75fr]"><article className="rounded-2xl border border-sand-200 bg-white p-5 shadow-premium-sm"><h2 className="font-display text-[18px] font-semibold text-ink-950">Ekonomisk utveckling</h2><div className="mt-3 flex gap-4 text-xs text-ink-500"><Legend cls="bg-petroleum-800" text="Hyresintäkter"/><Legend cls="bg-petroleum-400" text="Driftnetto"/><Legend cls="bg-sand-500" text="Kassaflöde"/></div>{series.every(v=>!v.income&&!v.net&&!v.cash)?<Empty icon={LineChart} title="Ingen ekonomisk historik ännu"/>:<div className="mt-5 rounded-xl bg-surface-subtle px-3 py-4"><svg viewBox="0 0 720 240" className="h-[230px] w-full" role="img" aria-label="Ekonomisk utveckling">{[0,1,2,3,4].map(i=><line key={i} x1="24" y1={20+i*48} x2="696" y2={20+i*48} stroke="#ece4d8"/>)}<path d={line(series.map(v=>v.income),chartMax)} fill="none" stroke="#29463f" strokeWidth="3"/><path d={line(series.map(v=>Math.max(0,v.net)),chartMax)} fill="none" stroke="#779e90" strokeWidth="2.5"/><path d={line(series.map(v=>Math.max(0,v.cash)),chartMax)} fill="none" stroke="#ad9f89" strokeWidth="2.25"/></svg><div className="grid grid-cols-6 text-center text-xs text-ink-400">{series.map(v=><span key={v.label}>{v.label}</span>)}</div></div>}</article><article className="rounded-2xl border border-sand-200 bg-white p-5 shadow-premium-sm"><div className="flex justify-between"><h2 className="font-display text-[18px] font-semibold text-ink-950">Budget vs utfall</h2><Link href="/dashboard/budget" className="text-xs font-semibold text-petroleum-700">Öppna budget</Link></div><div className="mt-5 divide-y divide-sand-100">{budgetRows.map(r=><BudgetRow key={r.label}{...r}/>)}</div></article></section>
    <section className="grid gap-4 xl:grid-cols-[1.25fr_0.85fr_0.8fr]"><article className="overflow-hidden rounded-2xl border border-sand-200 bg-white shadow-premium-sm"><Head title="Senaste avier" href="/dashboard/hyresavisering"/><p className="px-5 pt-3 text-xs text-ink-500">Översikt. Ändra aviestatus i kön ovan — avi-numret öppnar rätt avi.</p>{loading?<div className="space-y-2.5 p-4" aria-hidden="true">{[0,1,2,3,4].map(i=><div key={i} className="h-8 animate-pulse rounded-lg bg-sand-100"/>)}</div>:recent.length?<div className="overflow-x-auto"><table className="w-full min-w-[620px] text-left"><thead><tr className="bg-surface-subtle text-xs text-ink-400"><th className="px-4 py-2.5">Avie</th><th className="px-3">Motpart</th><th className="px-3">Fastighet</th><th className="px-3">Förfallodatum</th><th className="px-3 text-right">Belopp</th><th className="px-4">Status</th></tr></thead><tbody className="divide-y divide-sand-100">{recent.map(n=><tr key={n.id} className="text-xs text-ink-600 hover:bg-sand-50"><td className="px-4 py-3 font-medium"><Link href={`/dashboard/hyresavisering?id=${n.id}`} aria-label={`Öppna avie för ${n.tenant_name||"motpart"}`} className="text-ink-900 hover:underline">F-{n.period?.slice(0,4)||currentYear}-{n.id.slice(0,6).toUpperCase()}</Link></td><td className="px-3">{n.tenant_name||"Ej angiven"}</td><td className="px-3">{n.property_name||"—"}</td><td className="px-3">{n.due_date?dateFmt.format(new Date(n.due_date)):"—"}</td><td className="px-3 text-right font-semibold">{money.format(Number(n.total||0))}</td><td className="px-4"><Status status={n.status||"draft"}/></td></tr>)}</tbody></table></div>:<Empty icon={FileText} title="Inga avier ännu"/>}</article><article className="rounded-2xl border border-sand-200 bg-white p-5 shadow-premium-sm"><h2 className="font-display text-[17px] font-semibold">Fördelning per fastighet</h2>{distribution.length?<><div className="mx-auto mt-5 h-36 w-36 rounded-full" style={{background:donut(distribution)}}><div className="relative left-[26px] top-[26px] flex h-[92px] w-[92px] items-center justify-center rounded-full bg-white text-center text-xs font-semibold">{compact(distTotal)}<br/>Hyresintäkter</div></div><div className="mt-4 space-y-2">{distribution.map((v,i)=><Link key={v.name} href={v.id?`/dashboard/fastigheter/${v.id}`:"/dashboard/fastigheter"} className="flex items-center gap-2 text-xs text-ink-600"><span className={`h-2 w-2 rounded-full ${dot(i)}`}/><span className="flex-1 truncate">{v.name}</span><strong>{distTotal?((v.amount/distTotal)*100).toLocaleString("sv-SE",{maximumFractionDigits:1}):0}%</strong></Link>)}</div></>:<Empty icon={Building2} title="Ingen fördelning ännu"/>}</article><article className="rounded-2xl border border-sand-200 bg-white p-5 shadow-premium-sm"><Head title="Kommande förfallodatum" href="/dashboard/hyresavisering" compact/>{upcoming.length?<div className="mt-3 divide-y divide-sand-100">{upcoming.map(n=><Link key={n.id} href={`/dashboard/hyresavisering?id=${n.id}`} className="flex items-start gap-2 py-3 text-xs"><CalendarDays className="mt-0.5 h-4 w-4 text-petroleum-700"/><span className="min-w-0 flex-1"><strong className="block truncate">{n.tenant_name||"Hyresgäst"}</strong><span className="block truncate text-xs text-ink-400">{n.property_name||"Fastighet"} · {n.due_date?dateFmt.format(new Date(n.due_date)):"—"}</span></span><strong>{money.format(Number(n.total||0))}</strong></Link>)}</div>:<Empty icon={CalendarDays} title="Inga kommande avier"/>}</article></section>
  </div>;
}

function Kpi({icon:Icon,label,value,helper,href,negative=false}:{icon:LucideIcon;label:string;value:string;helper:string;href:string;negative?:boolean}){return <article className="rounded-2xl border border-sand-200 bg-white p-5 shadow-premium-sm"><div className="flex gap-3"><span className="flex h-11 w-11 items-center justify-center rounded-full bg-sand-50 text-petroleum-800"><Icon className="h-[18px] w-[18px]"/></span><div><p className="text-xs text-ink-650">{label}</p><p className="mt-1 font-display text-[27px] font-semibold text-ink-950">{value}</p><p className={`mt-1 text-xs ${negative?"text-danger-600":"text-petroleum-600"}`}>{helper}</p></div></div><Link href={href} className="mt-4 inline-flex items-center gap-1 text-xs font-semibold text-petroleum-700">Visa detaljer <ArrowRight className="h-3 w-3"/></Link></article>}
function BudgetRow({label,actual,budget,variance}:{label:string;actual:number;budget:number;variance:number|null}){const ratio=budget?Math.min(100,Math.abs(actual/budget)*100):actual?100:0;return <div className="grid grid-cols-[1fr_68px_68px_50px] items-center gap-2 py-3 text-xs"><div><p className="font-medium">{label}</p><div className="mt-1 h-1.5 rounded-full bg-sand-100"><div className="h-full rounded-full bg-petroleum-700" style={{width:`${ratio}%`}}/></div></div><span className="text-right">{compact(actual)}</span><span className="text-right text-ink-500">{compact(budget)}</span><span className={`text-right font-semibold ${(variance??0)<0?"text-danger-600":"text-petroleum-600"}`}>{variance===null?"—":`${variance>0?"+":""}${variance.toLocaleString("sv-SE",{maximumFractionDigits:1})}%`}</span></div>}
...
function Empty({icon:Icon,title}:{icon:LucideIcon;title:string}){return <div className="py-10 text-center"><Icon className="mx-auto h-7 w-7 text-sand-400"/><p className="mt-2 text-xs font-semibold text-ink-700">{title}</p></div>}
function Head({title,href,compact=false}:{title:string;href:string;compact?:boolean}){return <div className={`flex items-center justify-between ${compact?"":"border-b border-sand-100 px-5 py-4"}`}><h2 className="font-display text-[17px] font-semibold">{title}</h2><Link href={href} className="inline-flex items-center gap-1 text-xs font-semibold text-petroleum-700">Visa alla <ArrowRight className="h-3 w-3"/></Link></div>}
function Status({status}:{status:string}){const cls=status==="paid"?"border-success-200 bg-success-50 text-success-700":status==="overdue"?"border-danger-200 bg-danger-50 text-danger-700":"border-sand-200 bg-sand-50 text-ink-550";return <span className={`rounded-full border px-2 py-1 text-xs font-semibold ${cls}`}>{statusText[status]||status}</span>}
function donut(items:Array<{amount:number}>){const colors:readonly string[]=CHART_COLORS;const total=items.reduce((s,v)=>s+v.amount,0)||1;let p=0;return `conic-gradient(${items.map((v,i)=>{const a=p;p+=(v.amount/total)*100;return `${colors[i%colors.length]} ${a}% ${p}%`}).join(",")})`}
