...
export default function MaintenanceReportPage({params}:{params:Promise<{id:string}>}){
  ...
  return <main className="min-h-screen bg-sand-50 px-5 py-8 text-ink-900 print:bg-white print:p-0">
    <article className="mx-auto max-w-[1100px] rounded-2xl border border-sand-200 bg-white p-8 shadow-sm print:max-w-none print:border-0 print:p-0 print:shadow-none sm:p-12">
      ...
      {plan?<>
      <header className="border-b border-sand-200 pb-8"><p className="text-xs font-semibold uppercase tracking-[0.18em] text-petroleum-700">Revalta</p><h1 className="mt-3 text-4xl font-semibold tracking-[-0.04em]">Underhållsplan</h1><p className="mt-3 text-xl text-ink-600">{data?.property.name}</p><div className="mt-6 grid gap-4 text-sm sm:grid-cols-4"><Fact label="Plan" value={`${plan.name} · v${plan.version}`}/><Fact label="Period" value={`${plan.base_year}–${plan.base_year+plan.horizon_years-1}`}/><Fact label="Indexering" value={`${plan.annual_index_rate}% per år`}/><Fact label="Planerat behov" value={money.format(total)}/></div></header>

      <section className="mt-10"><h2 className="text-2xl font-semibold">Sammanfattning</h2><p className="mt-3 max-w-4xl text-sm leading-7 text-ink-600">{plan.summary||"Underhållsplanen sammanställer fastighetens långsiktiga åtgärder, risker och investeringsbehov."}</p>{plan.assumptions?<p className="mt-3 text-sm leading-7 text-ink-500"><strong>Antaganden:</strong> {plan.assumptions}</p>:null}</section>
