...
export function ImdPage({ initialCreate }: { initialCreate: boolean }) {
  ...
  return <div className="space-y-8">
    <PageHeader
      ...
      action={<div className="flex flex-wrap gap-2">{visibleReadings.length ? <button type="button" onClick={exportCsv} className={premiumSecondaryButtonClass}><Download className="mr-2 h-4 w-4" aria-hidden="true" />CSV</button> : null}{canManage || showCreate ? <button type="button" onClick={() => (showCreate ? closeCreate() : setShowCreate(true))} className={showCreate ? premiumSecondaryButtonClass : premiumPrimaryButtonClass}><Plus className="mr-2 h-4 w-4" aria-hidden="true" />{showCreate ? "Stäng" : "Ny avläsning"}</button> : null}</div>}
    />
