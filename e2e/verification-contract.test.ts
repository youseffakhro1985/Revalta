...
describe("authenticated Preview evidence", () => {
  ...
  it("requires owner unknown public ticket comments to stay not found without leftover errorCode", () => {
    expect(() => validateOwnerUnknownPublicTicketCommentNotFound(404, { error: "Ärendet hittades inte. Kontrollera referensnummer och e-post." })).not.toThrow();
    expect(() => validateOwnerUnknownPublicTicketCommentNotFound(200, { success: true })).toThrow(
      /unknown public ticket comment did not stay not found/,
    );
    expect(() => validateOwnerUnknownPublicTicketCommentNotFound(404, { error: "Ärendet hittades inte. Kontrollera referensnummer och e-post.", errorCode: "NOT_FOUND" })).toThrow(
      /unknown public ticket comment did not stay not found/,
    );
    expect(() => validateOwnerUnknownPublicTicketCommentNotFound(400, { error: "E-post eller spårningstoken och kommentar krävs" })).toThrow(
      /unknown public ticket comment did not stay not found/,
    );
    expect(() => validateOwnerUnknownPublicTicketCommentNotFound(404, { error: "Ärendet hittades inte" })).toThrow(
      /unknown public ticket comment did not stay not found/,
    );
  });

  it("requires owner unknown public ticket attachments to stay not found without leftover errorCode", () => {
    expect(() => validateOwnerUnknownPublicTicketAttachmentNotFound(404, { error: "Ärendet hittades inte. Kontrollera referensnummer och e-post." })).not.toThrow();
    expect(() => validateOwnerUnknownPublicTicketAttachmentNotFound(200, { success: true })).toThrow(
      /unknown public ticket attachment did not stay not found/,
    );
    expect(() => validateOwnerUnknownPublicTicketAttachmentNotFound(404, { error: "Ärendet hittades inte. Kontrollera referensnummer och e-post.", errorCode: "NOT_FOUND" })).toThrow(
      /unknown public ticket attachment did not stay not found/,
    );
    expect(() => validateOwnerUnknownPublicTicketAttachmentNotFound(400, { error: "Fil krävs" })).toThrow(
      /unknown public ticket attachment did not stay not found/,
    );
    expect(() => validateOwnerUnknownPublicTicketAttachmentNotFound(400, { error: "E-post eller spårningstoken krävs" })).toThrow(
      /unknown public ticket attachment did not stay not found/,
    );
  });

  it("requires owner unknown public ticket feedback to stay not found without leftover errorCode", () => {
    expect(() => validateOwnerUnknownPublicTicketFeedbackNotFound(404, { error: "Ärendet hittades inte. Kontrollera referensnummer och e-post." })).not.toThrow();
    expect(() => validateOwnerUnknownPublicTicketFeedbackNotFound(200, { success: true })).toThrow(
      /unknown public ticket feedback did not stay not found/,
    );
    expect(() => validateOwnerUnknownPublicTicketFeedbackNotFound(404, { error: "Ärendet hittades inte. Kontrollera referensnummer och e-post.", errorCode: "NOT_FOUND" })).toThrow(
      /unknown public ticket feedback did not stay not found/,
    );
    expect(() => validateOwnerUnknownPublicTicketFeedbackNotFound(400, { error: "E-post eller spårningstoken och betyg 1–5 krävs" })).toThrow(
      /unknown public ticket feedback did not stay not found/,
    );
    expect(() => validateOwnerUnknownPublicTicketFeedbackNotFound(409, { error: "Återkoppling kan lämnas när ärendet är avslutat." })).toThrow(
      /unknown public ticket feedback did not stay not found/,
    );
  });

  it("requires owner unknown resident portal tickets to stay not found with errorCode NOT_FOUND", () => {
    expect(() => validateOwnerUnknownResidentPortalTicketNotFound(404, { error: "Ärendet hittades inte", errorCode: "NOT_FOUND" })).not.toThrow();
    expect(() => validateOwnerUnknownResidentPortalTicketNotFound(200, { ticket: { id: "synthetic" } })).toThrow(
      /unknown resident portal ticket did not stay not found/,
    );
    expect(() => validateOwnerUnknownResidentPortalTicketNotFound(200, { success: true })).toThrow(
      /unknown resident portal ticket did not stay not found/,
    );
    expect(() => validateOwnerUnknownResidentPortalTicketNotFound(404, { error: "Ärendet hittades inte" })).toThrow(
      /unknown resident portal ticket did not stay not found/,
    );
    expect(() => validateOwnerUnknownResidentPortalTicketNotFound(403, { error: "Du saknar behörighet till boendeportalen", errorCode: "FORBIDDEN" })).toThrow(
      /unknown resident portal ticket did not stay not found/,
    );
    expect(() => validateOwnerUnknownResidentPortalTicketNotFound(404, { error: "Ärendet hittades inte. Kontrollera referensnummer och e-post.", errorCode: "NOT_FOUND" })).toThrow(
      /unknown resident portal ticket did not stay not found/,
    );
  });

  it("requires owner unknown resident portal ticket comments to stay not found with errorCode NOT_FOUND", () => {
    expect(() => validateOwnerUnknownResidentPortalTicketCommentNotFound(404, { error: "Ärendet hittades inte", errorCode: "NOT_FOUND" })).not.toThrow();
    expect(() => validateOwnerUnknownResidentPortalTicketCommentNotFound(201, { success: true, comment: { id: "synthetic" } })).toThrow(
      /unknown resident portal ticket comment did not stay not found/,
    );
    expect(() => validateOwnerUnknownResidentPortalTicketCommentNotFound(404, { error: "Ärendet hittades inte" })).toThrow(
      /unknown resident portal ticket comment did not stay not found/,
    );
    expect(() => validateOwnerUnknownResidentPortalTicketCommentNotFound(400, { error: "Kommentaren får inte vara tom", errorCode: "VALIDATION_FAILED" })).toThrow(
      /unknown resident portal ticket comment did not stay not found/,
    );
    expect(() => validateOwnerUnknownResidentPortalTicketCommentNotFound(403, { error: "Du saknar behörighet att kommentera", errorCode: "FORBIDDEN" })).toThrow(
      /unknown resident portal ticket comment did not stay not found/,
    );
  });

  it("requires owner unknown resident portal document download to stay not found with errorCode NOT_FOUND", () => {
    expect(() => validateOwnerUnknownResidentPortalDocumentDownloadNotFound(404, { error: "Dokumentet hittades inte", errorCode: "NOT_FOUND" })).not.toThrow();
    expect(() => validateOwnerUnknownResidentPortalDocumentDownloadNotFound(200, { success: true })).toThrow(
      /unknown resident portal document download did not stay not found/,
    );
    expect(() => validateOwnerUnknownResidentPortalDocumentDownloadNotFound(404, { error: "Dokumentet hittades inte" })).toThrow(
      /unknown resident portal document download did not stay not found/,
    );
    expect(() => validateOwnerUnknownResidentPortalDocumentDownloadNotFound(400, { error: "Hyresavtal krävs", errorCode: "VALIDATION_FAILED" })).toThrow(
      /unknown resident portal document download did not stay not found/,
    );
    expect(() => validateOwnerUnknownResidentPortalDocumentDownloadNotFound(403, { error: "Du saknar behörighet till boendedokument", errorCode: "FORBIDDEN" })).toThrow(
      /unknown resident portal document download did not stay not found/,
    );
  });

  it("requires owner unknown property maintenance plan export to stay not found without leftover errorCode", () => {
    expect(() => validateOwnerUnknownPropertyMaintenancePlanExportNotFound(404, { error: "Fastigheten hittades inte" })).not.toThrow();
    expect(() => validateOwnerUnknownPropertyMaintenancePlanExportNotFound(200, { success: true })).toThrow(
      /unknown property maintenance plan export did not stay not found/,
    );
    expect(() => validateOwnerUnknownPropertyMaintenancePlanExportNotFound(404, { error: "Fastigheten hittades inte", errorCode: "NOT_FOUND" })).toThrow(
      /unknown property maintenance plan export did not stay not found/,
    );
    expect(() => validateOwnerUnknownPropertyMaintenancePlanExportNotFound(404, { error: "Ingen underhållsplan finns" })).toThrow(
      /unknown property maintenance plan export did not stay not found/,
    );
    expect(() => validateOwnerUnknownPropertyMaintenancePlanExportNotFound(403, { error: "Du saknar behörighet" })).toThrow(
      /unknown property maintenance plan export did not stay not found/,
    );
  });

  it("requires owner unknown property maintenance plan governance to stay not found without leftover errorCode", () => {
    expect(() => validateOwnerUnknownPropertyMaintenancePlanGovernanceNotFound(404, { error: "Fastigheten hittades inte" })).not.toThrow();
    expect(() => validateOwnerUnknownPropertyMaintenancePlanGovernanceNotFound(200, { plans: [] })).toThrow(
      /unknown property maintenance plan governance did not stay not found/,
    );
    expect(() => validateOwnerUnknownPropertyMaintenancePlanGovernanceNotFound(200, { success: true })).toThrow(
      /unknown property maintenance plan governance did not stay not found/,
    );
    expect(() => validateOwnerUnknownPropertyMaintenancePlanGovernanceNotFound(404, { error: "Fastigheten hittades inte", errorCode: "NOT_FOUND" })).toThrow(
      /unknown property maintenance plan governance did not stay not found/,
    );
    expect(() => validateOwnerUnknownPropertyMaintenancePlanGovernanceNotFound(404, { error: "Planversionen hittades inte" })).toThrow(
      /unknown property maintenance plan governance did not stay not found/,
    );
  });

  it("requires owner unknown property maintenance plan action to stay not found without leftover errorCode", () => {
    expect(() => validateOwnerUnknownPropertyMaintenancePlanActionNotFound(404, { error: "Fastigheten hittades inte" })).not.toThrow();
    expect(() => validateOwnerUnknownPropertyMaintenancePlanActionNotFound(200, { success: true })).toThrow(
      /unknown property maintenance plan action did not stay not found/,
    );
    expect(() => validateOwnerUnknownPropertyMaintenancePlanActionNotFound(404, { error: "Fastigheten hittades inte", errorCode: "NOT_FOUND" })).toThrow(
      /unknown property maintenance plan action did not stay not found/,
    );
    expect(() => validateOwnerUnknownPropertyMaintenancePlanActionNotFound(400, { error: "Kontrollera åtgärdens namn, år och kostnad" })).toThrow(
      /unknown property maintenance plan action did not stay not found/,
    );
    expect(() => validateOwnerUnknownPropertyMaintenancePlanActionNotFound(404, { error: "Åtgärden hittades inte" })).toThrow(
      /unknown property maintenance plan action did not stay not found/,
    );
  });

  it("requires owner unknown property components overview reads to stay not found with leftover NOT_FOUND", () => {
    expect(() => validateOwnerUnknownPropertyComponentsOverviewNotFound(404, { error: "Fastigheten hittades inte", errorCode: "NOT_FOUND" })).not.toThrow();
    expect(() => validateOwnerUnknownPropertyComponentsOverviewNotFound(200, { summary: { total: 0 } })).toThrow(
      /unknown property components overview did not stay not found/,
    );
    expect(() => validateOwnerUnknownPropertyComponentsOverviewNotFound(200, { success: true })).toThrow(
      /unknown property components overview did not stay not found/,
    );
    expect(() => validateOwnerUnknownPropertyComponentsOverviewNotFound(404, { error: "Fastigheten hittades inte" })).toThrow(
      /unknown property components overview did not stay not found/,
    );
    expect(() => validateOwnerUnknownPropertyComponentsOverviewNotFound(404, { error: "Komponenten hittades inte", errorCode: "NOT_FOUND" })).toThrow(
      /unknown property components overview did not stay not found/,
    );
  });

  it("requires owner unknown property components manage writes to stay not found with leftover NOT_FOUND", () => {
    expect(() => validateOwnerUnknownPropertyComponentsManageNotFound(404, { error: "Fastigheten hittades inte", errorCode: "NOT_FOUND" })).not.toThrow();
    expect(() => validateOwnerUnknownPropertyComponentsManageNotFound(200, { success: true })).toThrow(
      /unknown property components manage did not stay not found/,
    );
    expect(() => validateOwnerUnknownPropertyComponentsManageNotFound(404, { error: "Fastigheten hittades inte" })).toThrow(
      /unknown property components manage did not stay not found/,
    );
    expect(() => validateOwnerUnknownPropertyComponentsManageNotFound(400, { error: "Ogiltig förfrågan", errorCode: "VALIDATION_FAILED" })).toThrow(
      /unknown property components manage did not stay not found/,
    );
    expect(() => validateOwnerUnknownPropertyComponentsManageNotFound(400, { error: "Välj en komponent", errorCode: "VALIDATION_FAILED" })).toThrow(
      /unknown property components manage did not stay not found/,
    );
    expect(() => validateOwnerUnknownPropertyComponentsManageNotFound(404, { error: "Komponenten hittades inte", errorCode: "NOT_FOUND" })).toThrow(
      /unknown property components manage did not stay not found/,
    );
  });

  it("requires owner unknown property component report reads to stay not found with leftover NOT_FOUND", () => {
    expect(() => validateOwnerUnknownPropertyComponentReportNotFound(404, { error: "Fastigheten hittades inte", errorCode: "NOT_FOUND" })).not.toThrow();
    expect(() => validateOwnerUnknownPropertyComponentReportNotFound(200, { summary: { eventCount: 0 } })).toThrow(
      /unknown property component report did not stay not found/,
    );
    expect(() => validateOwnerUnknownPropertyComponentReportNotFound(200, { success: true })).toThrow(
      /unknown property component report did not stay not found/,
    );
    expect(() => validateOwnerUnknownPropertyComponentReportNotFound(404, { error: "Fastigheten hittades inte" })).toThrow(
      /unknown property component report did not stay not found/,
    );
    expect(() => validateOwnerUnknownPropertyComponentReportNotFound(404, { error: "Komponenten hittades inte", errorCode: "NOT_FOUND" })).toThrow(
      /unknown property component report did not stay not found/,
    );
  });

  it("requires owner unknown property component link-options reads to stay not found with leftover NOT_FOUND", () => {
    expect(() => validateOwnerUnknownPropertyComponentLinkOptionsNotFound(404, { error: "Fastigheten hittades inte", errorCode: "NOT_FOUND" })).not.toThrow();
    expect(() => validateOwnerUnknownPropertyComponentLinkOptionsNotFound(200, { workOrders: [], projects: [] })).toThrow(
      /unknown property component link options did not stay not found/,
    );
    expect(() => validateOwnerUnknownPropertyComponentLinkOptionsNotFound(200, { success: true })).toThrow(
      /unknown property component link options did not stay not found/,
    );
    expect(() => validateOwnerUnknownPropertyComponentLinkOptionsNotFound(404, { error: "Fastigheten hittades inte" })).toThrow(
      /unknown property component link options did not stay not found/,
    );
    expect(() => validateOwnerUnknownPropertyComponentLinkOptionsNotFound(403, { error: "Du saknar behörighet att länka komponenthistorik", errorCode: "FORBIDDEN" })).toThrow(
      /unknown property component link options did not stay not found/,
    );
    expect(() => validateOwnerUnknownPropertyComponentLinkOptionsNotFound(404, { error: "Komponenten hittades inte", errorCode: "NOT_FOUND" })).toThrow(
      /unknown property component link options did not stay not found/,
    );
  });

  it("requires owner unknown property component maintenance-settings reads to stay not found with leftover NOT_FOUND", () => {
    expect(() => validateOwnerUnknownPropertyComponentMaintenanceSettingsNotFound(404, { error: "Fastigheten hittades inte", errorCode: "NOT_FOUND" })).not.toThrow();
    expect(() => validateOwnerUnknownPropertyComponentMaintenanceSettingsNotFound(200, { settings: { id: "synthetic" } })).toThrow(
      /unknown property component maintenance settings did not stay not found/,
    );
    expect(() => validateOwnerUnknownPropertyComponentMaintenanceSettingsNotFound(200, { success: true })).toThrow(
      /unknown property component maintenance settings did not stay not found/,
    );
    expect(() => validateOwnerUnknownPropertyComponentMaintenanceSettingsNotFound(404, { error: "Fastigheten hittades inte" })).toThrow(
      /unknown property component maintenance settings did not stay not found/,
    );
    expect(() => validateOwnerUnknownPropertyComponentMaintenanceSettingsNotFound(404, { error: "Komponenten hittades inte", errorCode: "NOT_FOUND" })).toThrow(
      /unknown property component maintenance settings did not stay not found/,
    );
  });

  it("requires owner unknown property component maintenance-settings writes to stay not found with leftover NOT_FOUND", () => {
    expect(() => validateOwnerUnknownPropertyComponentMaintenanceSettingsPatchNotFound(404, { error: "Fastigheten hittades inte", errorCode: "NOT_FOUND" })).not.toThrow();
    expect(() => validateOwnerUnknownPropertyComponentMaintenanceSettingsPatchNotFound(200, { settings: { id: "synthetic" } })).toThrow(
      /unknown property component maintenance settings patch did not stay not found/,
    );
    expect(() => validateOwnerUnknownPropertyComponentMaintenanceSettingsPatchNotFound(200, { success: true })).toThrow(
      /unknown property component maintenance settings patch did not stay not found/,
    );
    expect(() => validateOwnerUnknownPropertyComponentMaintenanceSettingsPatchNotFound(404, { error: "Fastigheten hittades inte" })).toThrow(
      /unknown property component maintenance settings patch did not stay not found/,
    );
    expect(() => validateOwnerUnknownPropertyComponentMaintenanceSettingsPatchNotFound(400, { error: "Serviceintervallet måste vara 1–120 månader", errorCode: "VALIDATION_FAILED" })).toThrow(
      /unknown property component maintenance settings patch did not stay not found/,
    );
    expect(() => validateOwnerUnknownPropertyComponentMaintenanceSettingsPatchNotFound(404, { error: "Komponenten hittades inte", errorCode: "NOT_FOUND" })).toThrow(
      /unknown property component maintenance settings patch did not stay not found/,
    );
  });

  it("requires owner unknown property component actions writes to stay not found with leftover NOT_FOUND", () => {
    expect(() => validateOwnerUnknownPropertyComponentActionsNotFound(404, { error: "Fastigheten hittades inte", errorCode: "NOT_FOUND" })).not.toThrow();
    expect(() => validateOwnerUnknownPropertyComponentActionsNotFound(201, { event: { id: "synthetic" } })).toThrow(
      /unknown property component actions did not stay not found/,
    );
    expect(() => validateOwnerUnknownPropertyComponentActionsNotFound(200, { success: true })).toThrow(
      /unknown property component actions did not stay not found/,
    );
    expect(() => validateOwnerUnknownPropertyComponentActionsNotFound(404, { error: "Fastigheten hittades inte" })).toThrow(
      /unknown property component actions did not stay not found/,
    );
    expect(() => validateOwnerUnknownPropertyComponentActionsNotFound(400, { error: "Ogiltig händelsetyp", errorCode: "VALIDATION_FAILED" })).toThrow(
      /unknown property component actions did not stay not found/,
    );
    expect(() => validateOwnerUnknownPropertyComponentActionsNotFound(404, { error: "Komponenten hittades inte", errorCode: "NOT_FOUND" })).toThrow(
      /unknown property component actions did not stay not found/,
    );
  });

  it("requires owner unknown property component entry writes to stay not found with leftover NOT_FOUND", () => {
    expect(() => validateOwnerUnknownPropertyComponentEntryNotFound(404, { error: "Fastigheten hittades inte", errorCode: "NOT_FOUND" })).not.toThrow();
    expect(() => validateOwnerUnknownPropertyComponentEntryNotFound(200, { entry: { id: "synthetic" } })).toThrow(
      /unknown property component entry did not stay not found/,
    );
    expect(() => validateOwnerUnknownPropertyComponentEntryNotFound(200, { success: true })).toThrow(
      /unknown property component entry did not stay not found/,
    );
    expect(() => validateOwnerUnknownPropertyComponentEntryNotFound(404, { error: "Fastigheten hittades inte" })).toThrow(
      /unknown property component entry did not stay not found/,
    );
    expect(() => validateOwnerUnknownPropertyComponentEntryNotFound(400, { error: "Ogiltig posttyp", errorCode: "VALIDATION_FAILED" })).toThrow(
      /unknown property component entry did not stay not found/,
    );
    expect(() => validateOwnerUnknownPropertyComponentEntryNotFound(404, { error: "Händelsen hittades inte", errorCode: "NOT_FOUND" })).toThrow(
      /unknown property component entry did not stay not found/,
    );
    expect(() => validateOwnerUnknownPropertyComponentEntryNotFound(404, { error: "Komponenten hittades inte", errorCode: "NOT_FOUND" })).toThrow(
      /unknown property component entry did not stay not found/,
    );
  });

  it("requires owner unknown property component detail to stay not found without leftover errorCode", () => {
    expect(() => validateOwnerUnknownPropertyComponentDetailNotFound(404, { error: "Fastigheten hittades inte" })).not.toThrow();
    expect(() => validateOwnerUnknownPropertyComponentDetailNotFound(200, { component: { id: "synthetic" } })).toThrow(
      /unknown property component detail did not stay not found/,
    );
    expect(() => validateOwnerUnknownPropertyComponentDetailNotFound(200, { property: { id: "synthetic" }, events: [] })).toThrow(
      /unknown property component detail did not stay not found/,
    );
    expect(() => validateOwnerUnknownPropertyComponentDetailNotFound(200, { success: true })).toThrow(
      /unknown property component detail did not stay not found/,
    );
    expect(() => validateOwnerUnknownPropertyComponentDetailNotFound(404, { error: "Fastigheten hittades inte", errorCode: "NOT_FOUND" })).toThrow(
      /unknown property component detail did not stay not found/,
    );
    expect(() => validateOwnerUnknownPropertyComponentDetailNotFound(404, { error: "Komponenten hittades inte" })).toThrow(
      /unknown property component detail did not stay not found/,
    );
  });
  ...
  it("runs owner billing Preview proof inside verified login without a workflow YAML change", () => {
    ...
    expect(runner).toContain("validateOwnerUnknownPublicTicketCommentNotFound");
    expect(runner).toContain("validateOwnerUnknownPublicTicketAttachmentNotFound");
    expect(runner).toContain("validateOwnerUnknownPublicTicketFeedbackNotFound");
    expect(runner).toContain("validateOwnerUnknownResidentPortalTicketNotFound");
    expect(runner).toContain("validateOwnerUnknownResidentPortalTicketCommentNotFound");
    expect(runner).toContain("validateOwnerUnknownResidentPortalDocumentDownloadNotFound");
    expect(runner).toContain("validateOwnerUnknownPropertyMaintenancePlanExportNotFound");
    expect(runner).toContain("validateOwnerUnknownPropertyMaintenancePlanGovernanceNotFound");
    expect(runner).toContain("validateOwnerUnknownPropertyMaintenancePlanActionNotFound");
    expect(runner).toContain("validateOwnerUnknownPropertyComponentsOverviewNotFound");
    expect(runner).toContain("validateOwnerUnknownPropertyComponentsManageNotFound");
    expect(runner).toContain("validateOwnerUnknownPropertyComponentReportNotFound");
    expect(runner).toContain("validateOwnerUnknownPropertyComponentLinkOptionsNotFound");
    expect(runner).toContain("validateOwnerUnknownPropertyComponentMaintenanceSettingsNotFound");
    expect(runner).toContain("validateOwnerUnknownPropertyComponentMaintenanceSettingsPatchNotFound");
    expect(runner).toContain("validateOwnerUnknownPropertyComponentActionsNotFound");
    expect(runner).toContain("validateOwnerUnknownPropertyComponentEntryNotFound");
    expect(runner).toContain("validateOwnerUnknownPropertyComponentDetailNotFound");
    ...
    expect(runner).toContain("/api/public/tickets/00000000-0000-4000-8000-000000000089/comments");
    expect(runner).toContain("/api/public/tickets/00000000-0000-4000-8000-000000000090/attachments");
    expect(runner).toContain("/api/public/tickets/00000000-0000-4000-8000-000000000091/feedback");
    expect(runner).toContain("e2e-unknown-public-ticket-feedback@example.com");
    expect(runner).toContain("/api/resident-portal/tickets/00000000-0000-4000-8000-000000000092");
    expect(runner).toContain("/api/resident-portal/tickets/00000000-0000-4000-8000-000000000093/comments");
    expect(runner).toContain("/api/resident-portal/documents/00000000-0000-4000-8000-000000000094/download");
    expect(runner).toContain("leaseId=00000000-0000-4000-8000-000000000000");
    expect(runner).toContain("/api/properties/00000000-0000-4000-8000-000000000095/maintenance-plan/export");
    expect(runner).toContain("/api/properties/00000000-0000-4000-8000-000000000096/maintenance-plan/governance");
    expect(runner).toContain("/api/properties/00000000-0000-4000-8000-000000000097/maintenance-plan/action");
    expect(runner).toContain("/api/properties/00000000-0000-4000-8000-000000000098/components/overview");
    expect(runner).toContain("/api/properties/00000000-0000-4000-8000-000000000099/components/manage");
    expect(runner).toContain("/api/properties/00000000-0000-4000-8000-000000000100/components/00000000-0000-4000-8000-000000000200/report");
    expect(runner).toContain("/api/properties/00000000-0000-4000-8000-000000000101/components/00000000-0000-4000-8000-000000000201/link-options");
    expect(runner).toContain("/api/properties/00000000-0000-4000-8000-000000000102/components/00000000-0000-4000-8000-000000000202/maintenance-settings");
    expect(runner).toContain("/api/properties/00000000-0000-4000-8000-000000000103/components/00000000-0000-4000-8000-000000000203/maintenance-settings");
    expect(runner).toContain("/api/properties/00000000-0000-4000-8000-000000000104/components/00000000-0000-4000-8000-000000000204/actions");
    expect(runner).toContain("/api/properties/00000000-0000-4000-8000-000000000105/components/00000000-0000-4000-8000-000000000205/entries/event/00000000-0000-4000-8000-000000000305");
    expect(runner).toContain("/api/properties/00000000-0000-4000-8000-000000000106/components/00000000-0000-4000-8000-000000000206");
    expect(runner).toContain("E2E unknown component entry");
    expect(runner).toContain("E2E unknown component event");
    expect(runner).toContain("00000000-0000-4000-8000-000000000199");
    expect(runner).toContain("e2e-unknown-public-ticket-attachment@example.com");
    expect(runner).toContain("e2e-unknown-public-ticket-attachment.txt");
    expect(runner).toContain("e2e-unknown-public-ticket-comment@example.com");
    expect(runner).toContain("E2E unknown public ticket comment");
