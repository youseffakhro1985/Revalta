...
export function InspectionWorkOrderCenter() {
  ...
  useEffect(() => {
    if (loading) return;
    if (window.location.hash !== "#punktlista") return;
    document.getElementById("punktlista")?.scrollIntoView({ behavior: "smooth", block: "start" });
    window.setTimeout(() => document.getElementById("punktlista-uppdatera")?.focus(), 0);
  }, [loading, record]);
