  ...
  return <div className="space-y-4 sm:space-y-5">
    ...
    <section className="rounded-2xl border border-sand-200 bg-white p-3 shadow-premium-sm">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
        <label className="relative block w-full sm:max-w-2xl">
        ...
        <button type="button" onClick={() => { setFocus("all"); setQuery(""); }} disabled={focus === "all" && !query} className="h-11 shrink-0 rounded-xl border border-sand-200 bg-white px-4 text-[11px] font-semibold text-ink-600 transition hover:bg-sand-50 disabled:cursor-not-allowed disabled:opacity-45">Rensa filter</button>
      </div>
    </section>
    ...
    <section className="overflow-hidden rounded-2xl border border-sand-200 bg-white shadow-premium-sm" aria-label="Snabbfilter för arbetsordrar">
      <div className="grid divide-y divide-sand-100 sm:grid-cols-2 sm:divide-y-0 xl:grid-cols-4 xl:divide-x xl:divide-sand-100">
        <Kpi icon={AlertTriangle} label="Öppna arbetsorder" value={loading ? "—" : activeOrders.length} helper={`${overdue.length} kräver SLA-fokus`} onClick={() => setFocus(focus === "open" ? "all" : "open")} active={focus === "open"} tone="danger" />
        <Kpi icon={Clock3} label="Akuta ärenden" value={loading ? "—" : urgentOrders.length} helper="Akut prioritet eller akut arbetstyp" onClick={() => setFocus(focus === "urgent" ? "all" : "urgent")} active={focus === "urgent"} tone="warning" />
        <Kpi icon={PlayCircle} label="Pågående idag" value={loading ? "—" : inProgress.length} helper={`${todayPlanning.length} schemalagda idag`} onClick={() => setFocus(focus === "active" ? "all" : "active")} active={focus === "active"} tone="info" />
        <Kpi icon={CheckCircle2} label="Slutförda denna vecka" value={loading ? "—" : completedThisWeek.length} helper="Klara eller fakturerade" onClick={() => setFocus(focus === "completed" ? "all" : "completed")} active={focus === "completed"} tone="success" />
      </div>
    </section>
    ...
    <section className="grid gap-4 xl:grid-cols-[1.2fr_0.68fr_0.82fr]">
      <article className="rounded-2xl border border-sand-200 bg-white p-5 shadow-premium-sm">
        ...
        <div className="mt-4 rounded-xl bg-[#FCFBF8] px-3 py-4">
          ...
          <div className="grid grid-cols-8 text-center text-[10px] text-ink-400">{weekly.map((point) => <span key={point.label}>{point.label}</span>)}</div>
        </div>
      ...
      <article className="overflow-hidden rounded-2xl border border-sand-200 bg-white shadow-premium-sm">
        <div className="flex items-center justify-between px-4 py-3.5"><div><h2 className="font-display text-[17px] font-semibold text-ink-950">Arbetsorder på karta</h2><p className="mt-0.5 text-[10px] text-ink-450">Klicka mellan arbetsorder och planera rutt.</p></div><MapPin className="h-4 w-4 text-petroleum-700" /></div>
        ...
        {selectedMapOrder ? <>
          ...
          <div className="border-t border-sand-100 p-3">
            ...
            <div className="grid grid-cols-2 gap-2"><a href={mapExternal} target="_blank" rel="noreferrer" className="inline-flex h-9 items-center justify-center gap-1.5 rounded-xl border border-sand-200 bg-white px-2 text-[10px] font-semibold text-petroleum-700 hover:bg-petroleum-50">Öppna karta <ExternalLink className="h-3 w-3" /></a><a href={routeUrl(mapOrders)} target="_blank" rel="noreferrer" className="inline-flex h-9 items-center justify-center gap-1.5 rounded-xl bg-petroleum-900 px-2 text-[10px] font-semibold text-white hover:bg-petroleum-800"><Route className="h-3.5 w-3.5" />Planera rutt</a></div>
          </div>
    ...
    <section className="grid gap-4 xl:grid-cols-[1.24fr_0.78fr_0.86fr]">
      ...
      <article className="rounded-2xl border border-sand-200 bg-white shadow-premium-sm">
        ...
        {todayPlanning.length ? <div className="divide-y divide-sand-100 px-4">{todayPlanning.map((order) => <Link key={order.id} href={`/dashboard/arbetsorder/${order.id}`} className="grid grid-cols-[44px_32px_minmax(0,1fr)_auto] items-center gap-2 py-3 transition hover:bg-petroleum-50/40"><span className="text-[10px] font-semibold text-ink-500">{timeFmt.format(new Date(order.scheduled_start || ""))}</span><span className="flex h-8 w-8 items-center justify-center rounded-full bg-petroleum-50 text-[10px] font-semibold text-petroleum-800">{(order.assigned_to?.name || order.assigned_to?.email || "ET").split(/\s|@/).slice(0, 2).map((part) => part[0]).join("").toUpperCase()}</span><span className="min-w-0"><span className="block truncate text-[10px] font-semibold text-ink-850">{order.assigned_to?.name || order.assigned_to?.email || "Ej tilldelad"}</span><span className="mt-0.5 block truncate text-[10px] text-ink-450">{order.property.name} · {order.title}</span></span><ArrowRight className="h-3.5 w-3.5 text-petroleum-700" /></Link>)}</div> : <Empty title={loading ? "Läser dagens planering…" : "Inga arbetsorder schemalagda idag"} />}
...
function Kpi({ icon: Icon, label, value, helper, onClick, active, tone }: { icon: LucideIcon; label: string; value: string | number; helper: string; onClick: () => void; active: boolean; tone: "danger" | "warning" | "info" | "success" }) {
  const iconTone = tone === "danger" ? "bg-danger-50 text-danger-700" : tone === "warning" ? "bg-warning-50 text-warning-700" : tone === "info" ? "bg-petroleum-50 text-petroleum-700" : "bg-success-50 text-success-700";
  return (
    <button
      type="button"
      onClick={onClick}
      aria-pressed={active}
      className={`group relative flex items-start gap-3 px-5 py-4 text-left outline-none transition focus-visible:bg-petroleum-50/60 ${active ? "bg-petroleum-50/50" : "hover:bg-sand-50/70"}`}
    >
      <span className={`absolute inset-x-0 top-0 h-0.5 transition ${active ? "bg-petroleum-800" : "bg-transparent"}`} aria-hidden />
      ...
      <span className="min-w-0">
        <span className="block text-[10px] font-semibold uppercase tracking-[0.09em] text-ink-450">{label}</span>
        <span className="mt-1.5 block font-display text-[26px] font-semibold leading-none tracking-[-0.04em] text-ink-950">{value}</span>
        <span className="mt-1.5 block truncate text-[10px] text-ink-400">{helper}</span>
      </span>
    </button>
  );
}
...
function MapChip({ label, dot, onClick }: { label: string; dot: string; onClick: () => void }) {
  return <button type="button" onClick={onClick} className="inline-flex items-center gap-1.5 rounded-full border border-sand-200 bg-white px-2.5 py-1 text-[10px] font-semibold text-ink-650 hover:bg-sand-50"><span className={`h-2 w-2 rounded-full ${dot}`} />{label}</button>;
}
