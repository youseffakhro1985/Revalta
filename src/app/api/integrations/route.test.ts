...
describe("integrations summary", () => {
  ...
  it("denies technicians from reading integration env keys and events", async () => {
    getCurrentUserMock.mockResolvedValue({ id: "tech-1", company_id: "company-1", role: "technician" });
    const response = await GET();
    expect(response.status).toBe(403);
    expect((await response.json()).error).toBe("Du saknar behörighet att visa integrationer");
    expect(integrationEventFindManyMock).not.toHaveBeenCalled();
    expect(invoiceJobGroupByMock).not.toHaveBeenCalled();
  });

  it("rejects residents before listing integration events", async () => {
