...
| Projects / maintenance / components | Company filters present | Property joins in several engines; project GET by session `company_id`; maintenance POST re-reads property via `tenantWhere`; component writes re-read property via `tenantWhere` and related WO/project/lifecycle ids in caller company; manage looks up asset in caller company | Project POST Tenant B property/manager/WO 404; project GET Tenant B id 404; maintenance POST Tenant B propertyId 404; PATCH Tenant B item id 404; component PATCH/action/report/entry/manage Tenant B property/asset/project/lifecycle/cost 404 | Dedicated technician-role Preview fixture still P1 |
| Documents / operational documents | Company filters; parent constraint migration exists | Parent must be same company; download lookup by session `company_id`; list/POST resolve entity in caller company; work-order documents via `findAccessibleWorkOrder`; library filters keep caller `company_id` | Document download Tenant B 404 without blob; operational download/list/upload/delete Tenant B 404; WO GET/POST/download Tenant B 404; library Tenant B propertyId stays in caller company; managed upload Tenant B property/lease 404; PATCH Tenant B document id 404 | Remaining quote PDF/output if added |
