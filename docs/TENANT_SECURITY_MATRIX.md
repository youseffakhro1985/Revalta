...
| Documents / operational documents | Company filters; parent constraint migration exists | Parent must be same company | Document download Tenant B 404 without blob stream | Operational-document blob URL negatives still P1 |
| Quotes / vendors / insurance | Company filters present | Vendor create property via `findCompanyOwned` | Vendor POST Tenant B propertyId 404 | Remaining quote related-id tests |
