    ...
    <div className="space-y-5">
      ...
      {loading ? <div className="h-48 animate-pulse rounded-xl bg-sand-100" /> : !detail ? <EmptyState title="Välj ett avtal" description="Besiktningspunkter registreras separat per avtal och objekt." /> : <>
        ...
        <div className="space-y-4">{detail.record.items.length === 0 ? <EmptyState title="Inga besiktningspunkter" description="Lägg till rum, område eller byggnadsdel som ska kontrolleras." /> : detail.record.items.map((item, index) => <article key={item.id} className="rounded-2xl border border-sand-200 p-5">
          <div className="flex items-center justify-between"><h3 className="font-semibold text-ink-900">Besiktningspunkt {index + 1}</h3>{canEdit ? <button type="button" onClick={() => removeItem(item.id)} className="rounded-lg p-2 text-ink-500 hover:bg-danger-50 hover:text-danger-700" aria-label="Ta bort"><Trash2 className="h-4 w-4" /></button> : null}</div>
          <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4"><input disabled={!canEdit} className={premiumFieldClass} placeholder="Rum eller område" aria-label="Rum eller område" value={item.area} onChange={(event) => patchItem(item.id, { area: event.target.value })} /><input disabled={!canEdit} className={premiumFieldClass} placeholder="Byggnadsdel, t.ex. golv" aria-label="Byggnadsdel" value={item.component} onChange={(event) => patchItem(item.id, { component: event.target.value })} /><select disabled={!canEdit} className={premiumFieldClass} aria-label="Skick" value={item.condition} onChange={(event) => patchItem(item.id, { condition: event.target.value as InspectionCondition, selectedForWorkOrder: event.target.value === "action_required" ? item.selectedForWorkOrder : false })}>{Object.entries(conditionLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><select disabled={!canEdit} className={premiumFieldClass} aria-label="Prioritet" value={item.priority} onChange={(event) => patchItem(item.id, { priority: event.target.value as InspectionPriority })}>{Object.entries(priorityLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></div>
          <div className="mt-3 grid gap-3 md:grid-cols-2"><textarea disabled={!canEdit} className={premiumTextareaClass} placeholder="Beskriv skick eller anmärkning" aria-label="Beskriv skick eller anmärkning" value={item.description} onChange={(event) => patchItem(item.id, { description: event.target.value })} /><textarea disabled={!canEdit} className={premiumTextareaClass} placeholder="Rekommenderad åtgärd" aria-label="Rekommenderad åtgärd" value={item.recommendation} onChange={(event) => patchItem(item.id, { recommendation: event.target.value })} /></div>
