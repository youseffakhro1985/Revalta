...
export function MaintenancePlanPanel({propertyId}:{propertyId:string}){
  ...
  if(!loading && !data)return <InlineAlert>{error||"Underhållsplanen kunde inte laddas."}</InlineAlert>;
  ...
  return <section className="space-y-6" aria-labelledby="maintenance-heading">
    ...
    {card.activePlan&&card.forecast?<>
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {[5,10,20,30].map(years=><MetricCard key={years} icon={years===5?CircleDollarSign:TrendingUp} label={`${years}-årsbehov`} value={money.format(card.forecast!.totals[String(years)]||0)} hint={years<=card.activePlan!.horizon_years?`Från basår ${card.activePlan!.base_year}`:"Utanför vald horisont"}/>) }
      </div>
      <div className="grid gap-6 xl:grid-cols-[1.3fr_0.7fr]">
        <Panel title="Årsvis investeringsbehov" description={`Indexerat med ${card.activePlan.annual_index_rate}% per år.`}>
          {card.forecast.yearly.length===0?<EmptyState title="Inga kostnader planerade" description="Lägg till en underhållsåtgärd för att bygga prognosen."/>:<div className="space-y-3">{card.forecast.yearly.map(item=><div key={item.year} className="grid grid-cols-[52px_1fr_auto] items-center gap-3"><span className="text-xs font-semibold text-ink-500">{item.year}</span><div className="h-3 overflow-hidden rounded-full bg-sand-100"><div className="h-full rounded-full bg-petroleum-600" style={{width:`${Math.max(3,(item.amount/maxYear)*100)}%`}}/></div><span className="text-xs font-semibold text-ink-800">{money.format(item.amount)}</span></div>)}</div>}
        </Panel>
        <Panel title="Planstatus" description="Aktiv version och riskbild.">
          <dl className="space-y-4 text-sm"><Row label="Plan" value={`${card.activePlan.name} · v${card.activePlan.version}`}/><Row label="Tidshorisont" value={`${card.activePlan.horizon_years} år`}/><Row label="Åtgärder" value={String(card.actions.length)}/><Row label="Akuta/kritiska" value={String(card.forecast.urgent)}/><Row label="Försenade" value={String(card.forecast.overdue)}/></dl>
          {(card.forecast.urgent>0||card.forecast.overdue>0)?<div className="mt-5 flex gap-3 rounded-xl border border-warning-200 bg-warning-50 p-4 text-sm text-warning-900"><AlertTriangle className="h-5 w-5 shrink-0"/><p>{card.forecast.urgent} kritiska och {card.forecast.overdue} försenade åtgärder behöver följas upp.</p></div>:<div className="mt-5 flex gap-3 rounded-xl border border-petroleum-100 bg-petroleum-50 p-4 text-sm text-petroleum-900"><CheckCircle2 className="h-5 w-5 shrink-0"/><p>Planen saknar kritiska eller försenade åtgärder.</p></div>}
        </Panel>
      </div>
    ...
    <div className="grid gap-6 xl:grid-cols-[0.72fr_1.28fr]">
      ...
      <Panel title="Administrera plan" description="Skapa planversioner och registrera nya underhållsåtgärder.">
        ...
        <form onSubmit={submit} className="space-y-4">
          ...
          {mode==="plan"?<>
            <input type="hidden" name="action" value="plan.create"/><Field label="Planens namn"><input autoFocus required name="name" className={premiumFieldClass} placeholder="Underhållsplan 2026–2055"/></Field>
            ...
            <Field label="Byggnad"><select name="buildingId" className={premiumFieldClass}><option value="">Hela fastigheten</option>{card.property.buildings.map(b=><option key={b.id} value={b.id}>{b.name}</option>)}</select></Field>
            <Field label="Teknisk installation"><select name="technicalAssetId" className={premiumFieldClass}><option value="">Ingen särskild installation</option>{card.assets.map(a=><option key={a.id} value={a.id}>{a.name}</option>)}</select></Field>
          ...
          <button disabled={formLocked||(!card.activePlan&&card.plans.length===0&&mode==="action")} className={`${premiumPrimaryButtonClass} w-full`}><Plus className="h-4 w-4"/>{saving?"Sparar…":mode==="plan"?"Skapa planversion":"Lägg till åtgärd"}</button>
          </fieldset>
        </form>
        {card.plans.length>0?<div className="mt-6 border-t border-sand-100 pt-5"><p className="mb-3 text-xs font-semibold uppercase tracking-wide text-ink-500">Planversioner</p><div className="space-y-2">{card.plans.map(plan=><div key={plan.id} className="flex items-center justify-between gap-3 rounded-xl bg-sand-50 p-3"><div><p className="text-sm font-semibold text-ink-800">{plan.name} · v{plan.version}</p><p className="text-xs text-ink-500">{plan.base_year} · {plan.horizon_years} år</p></div>{plan.status==="active"?<Badge value="active" type="status"/>:<button type="button" disabled={formLocked} onClick={()=>activate(plan.id)} className="text-xs font-semibold text-petroleum-700">Aktivera</button>}</div>)}</div></div>:null}
      </Panel>
      </div>
