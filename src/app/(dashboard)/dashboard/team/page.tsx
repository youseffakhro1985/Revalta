...
export default function TeamPage() {
  ...
  return (
    <div className="mx-auto max-w-6xl animate-fade-in space-y-6">
      <div className="overflow-hidden rounded-2xl border border-sand-200 bg-white text-ink-950 shadow-premium-md">
        <div className="p-7 sm:p-8">
          <p className="mb-3 text-[11px] font-semibold uppercase tracking-[0.16em] text-petroleum-700">Team & behörigheter</p>
          <h1 className="text-[32px] font-semibold leading-tight tracking-[-0.035em] sm:text-[36px]">{companyName}</h1>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[0.85fr_1.15fr]">
        <section className="rounded-2xl border border-sand-200 bg-white p-7 shadow-premium-sm sm:p-8">
          <h2 className="text-[22px] font-semibold text-ink-950">Bjud in teammedlem</h2>
          ...
          <form onSubmit={handleSubmit} className="mt-6 space-y-5">
            <fieldset disabled={!canManage} className="space-y-5 disabled:opacity-60">
              <div>
                ...
                <input className="block w-full rounded-lg border border-sand-200 p-3 shadow-inner-sm outline-none focus:border-petroleum-500" value={name} onChange={(event) => setName(event.target.value)} placeholder="Ex. Sara Tekniker" />
              </div>
              <div>
                <label className="mb-1 block text-sm font-medium text-ink-700">E-post</label>
                <input type="email" required className="block w-full rounded-lg border border-sand-200 p-3 shadow-inner-sm outline-none focus:border-petroleum-500" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="sara@exempel.se" />
              </div>
              <div>
                <label className="mb-1 block text-sm font-medium text-ink-700">Roll</label>
                <select className="block w-full rounded-lg border border-sand-200 bg-white p-3 shadow-inner-sm outline-none focus:border-petroleum-500" value={role} onChange={(event) => setRole(event.target.value)}>
              ...
              <button disabled={submitting || !canManage} className="w-full rounded-lg bg-petroleum-700 px-8 py-3 font-semibold text-white shadow-premium-sm transition-all hover:bg-petroleum-700 disabled:cursor-not-allowed disabled:opacity-70">
          ...
          {inviteUrl && (
            <div className="mt-5 rounded-2xl border border-petroleum-100 bg-petroleum-50 p-4">
              <p className="text-sm font-semibold text-petroleum-700">Inbjudningslänk</p>
        ...
        <section className="overflow-hidden rounded-2xl border border-sand-200 bg-white shadow-premium-sm">
          <div className="border-b border-sand-100 bg-sand-50/70 p-6">
            <h2 className="text-lg font-semibold text-ink-950">Team</h2>
              ...
              {members.map((member) => (
                <article key={member.id} className="p-6">
                  <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                      <h3 className="text-lg font-semibold text-ink-950">{member.name || member.email}</h3>
                      <p className="mt-1 text-sm text-ink-500">{member.email}</p>
                    </div>
                    <div className="flex flex-wrap gap-2">
                      <span className="rounded-full border border-petroleum-100 bg-petroleum-50 px-3 py-1 text-xs font-semibold text-petroleum-600">{roleLabels[member.role] || member.role}</span>
                      <span className="rounded-full border border-sand-200 bg-sand-50 px-3 py-1 text-xs font-semibold text-ink-600">{member._count.assigned_tickets} tilldelade</span>
                    </div>
                  </div>
                </article>
              ))}
      ...
      <section className="overflow-hidden rounded-2xl border border-sand-200 bg-white shadow-premium-sm">
        <div className="border-b border-sand-100 bg-sand-50/70 p-6">
          <h2 className="text-lg font-semibold text-ink-950">Senaste inbjudningar</h2>
        ...
        <div className="divide-y divide-sand-100">
          {invites.length > 0 ? invites.map((invite) => (
            <article key={invite.id} className="p-6">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <h3 className="font-semibold text-ink-950">{invite.name || invite.email}</h3>
                  <p className="mt-1 text-sm text-ink-500">{invite.email}</p>
                </div>
                <div className="flex flex-wrap gap-2">
                  <span className="rounded-full border border-petroleum-100 bg-petroleum-50 px-3 py-1 text-xs font-semibold text-petroleum-600">{roleLabels[invite.role] || invite.role}</span>
                  <span className={`rounded-full border px-3 py-1 text-xs font-semibold ${invite.accepted_at ? "border-success-500 bg-success-50 text-success-600" : "border-warning-500 bg-warning-50 text-warning-600"}`}>
