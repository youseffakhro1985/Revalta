...
| Documents / operational documents | Company filters; parent constraint migration exists | Parent must be same company; download lookup by session `company_id`; list/POST resolve entity in caller company | Document download Tenant B 404 without blob stream; operational download Tenant B 404; untrusted storage URL is not fetched; GET list Tenant B property 404; POST upload Tenant B property 404 without blob write; DELETE Tenant B document id 404 | Remaining work-order document attach surfaces |
