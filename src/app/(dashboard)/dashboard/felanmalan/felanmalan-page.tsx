...
function statusTone(status: string) {
  if (status === "completed" || status === "closed") return "bg-success-50 text-success-700 ring-success-100";
  if (status === "in_progress") return "bg-warning-50 text-warning-800 ring-warning-100";
...
function priorityTone(priority: string) {
  if (priority === "urgent") return "bg-danger-50 text-danger-700 ring-danger-100";
  if (priority === "high") return "bg-warning-50 text-warning-700 ring-warning-100";
...
function KpiCard({ icon: Icon, label, value, hint, tone = "green" }: { icon: LucideIcon; label: string; value: number | string; hint: string; tone?: "green" | "red" | "amber" }) {
  const iconTone = tone === "red" ? "bg-danger-50 text-danger-600" : tone === "amber" ? "bg-warning-50 text-warning-600" : "bg-success-50 text-petroleum-700";
  const hintTone = tone === "red" ? "text-danger-600" : tone === "amber" ? "text-warning-600" : "text-success-700";
  return <article className="rounded-2xl border border-sand-200 bg-white p-5 shadow-premium-sm">
    <div className="flex items-start gap-4">
      ...
      <div className="min-w-0"><p className="text-xs font-medium text-ink-550">{label}</p><p className="mt-1 font-display text-[30px] font-semibold leading-none tracking-[-0.04em] text-ink-950">{value}</p><p className={`mt-3 text-xs font-semibold ${hintTone}`}>{hint}</p></div>
    </div>
  </article>;
}

export function FelanmalanPage({ initialCreate }: { initialCreate: boolean }) {
  ...
  useEffect(() => {
    ...
    if (window.location.hash !== "#arende-editor" && new URLSearchParams(window.location.search).get("create") !== "1") return;
    document.getElementById("arende-editor")?.scrollIntoView({ behavior: "smooth", block: "start" });
    window.setTimeout(() => document.getElementById("arende-titel")?.focus(), 0);
  }, [loading, createOpen]);
  useEffect(() => {
    if (loading) return;
    ...
    window.setTimeout(() => document.getElementById("senastearenden-alla")?.focus(), 0);
  }, [loading, tickets]);
  useEffect(() => {
    if (loading) return;
  ...
  return <div className="space-y-4 sm:space-y-5">
    <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
      <div>
        <p className="text-xs font-semibold uppercase tracking-[0.18em] text-petroleum-700">Drift / Ärenden</p>
      ...
      <div className="flex flex-wrap gap-2">
        {permissions.canExport ? (
          // CSV download hits the API, not an App Router page.
    ...
    <section className="grid gap-4 xl:grid-cols-[1.15fr_0.72fr_1fr]">
      <article className="rounded-2xl border border-sand-200 bg-white p-5 shadow-premium-sm">
        <div className="flex items-center justify-between gap-3"><div><h2 className="font-display text-[17px] font-semibold text-ink-950">Ärendeutveckling</h2><p className="mt-1 text-xs text-ink-450">Nya och slutförda ärenden över vald period.</p></div><span className="rounded-lg border border-sand-200 bg-surface-subtle px-2.5 py-1.5 text-xs font-semibold text-ink-550">{range} dagar</span></div>
        <div className="mt-4 flex gap-4 text-xs text-ink-500"><span className="flex items-center gap-1.5"><i className="h-1.5 w-4 rounded-full bg-petroleum-800" />Nya ärenden</span><span className="flex items-center gap-1.5"><i className="h-1.5 w-4 rounded-full bg-success-300" />Slutförda ärenden</span></div>
        <div className="mt-4 rounded-xl bg-surface-subtle px-3 py-4"><svg viewBox="0 0 640 210" className="h-[205px] w-full" role="img" aria-label="Ärendeutveckling">{[0, 1, 2, 3, 4].map((index) => <line key={index} x1="10" y1={12 + index * 44} x2="630" y2={12 + index * 44} stroke="#ece4d8" />)}<path d={linePath(trend.map((point) => point.created), trendMax)} fill="none" stroke="#29463f" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" /><path d={linePath(trend.map((point) => point.completed), trendMax)} fill="none" stroke="#93c7ae" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" /></svg><div className="grid grid-cols-5 text-center text-xs text-ink-400">{[0, .25, .5, .75, 1].map((factor) => { const index = Math.min(trend.length - 1, Math.round((trend.length - 1) * factor)); return <span key={factor}>{trend[index]?.label || "—"}</span>; })}</div></div>
      </article>
      ...
      <article className="rounded-2xl border border-sand-200 bg-white p-5 shadow-premium-sm">
        ...
        <div className="mt-5 flex flex-col items-center gap-5 sm:flex-row xl:flex-col 2xl:flex-row">
          <div className="relative h-36 w-36 shrink-0 rounded-full" style={{ background: statusDonut }}><div className="absolute inset-[22px] flex flex-col items-center justify-center rounded-full bg-white"><strong className="font-display text-[26px] text-ink-950">{overview?.summary.total ?? 0}</strong><span className="text-xs text-ink-450">Totalt</span></div></div>
          <div className="w-full space-y-2.5">{statusRows.slice(0, 5).map((row, index) => <div key={row.status} className="flex items-center gap-2 text-xs"><span className="h-2 w-2 rounded-full" style={{ backgroundColor: statusColors[index % statusColors.length] }} /><span className="min-w-0 flex-1 truncate text-ink-600">{statusLabels[row.status] || row.status}</span><strong className="text-ink-800">{row.count}</strong><span className="w-8 text-right text-ink-400">{row.percent}%</span></div>)}</div>
        </div>
      </article>

      <article className="overflow-hidden rounded-2xl border border-sand-200 bg-white shadow-premium-sm">
        <div className="flex items-center justify-between px-5 py-4"><div><h2 className="font-display text-[17px] font-semibold text-ink-950">Ärenden på karta</h2><p className="mt-0.5 text-xs text-ink-450">Välj ett aktivt ärende för plats och fastighet.</p></div><a href={mapExternal} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-xs font-semibold text-petroleum-700">Visa plats <ExternalLink className="h-3 w-3" /></a></div>
        {selectedMapTicket ? <><div className="relative h-[215px] bg-sand-100"><iframe key={selectedMapTicket.id} title={`Karta för ${selectedMapTicket.title}`} src={mapEmbed} className="h-full w-full border-0" loading="lazy" referrerPolicy="no-referrer-when-downgrade" allowFullScreen /><Link href={`/dashboard/felanmalan/${selectedMapTicket.id}`} className="absolute bottom-3 left-3 max-w-[75%] rounded-xl border border-white/80 bg-white/95 px-3 py-2 shadow-sm backdrop-blur-sm"><p className="truncate text-xs font-semibold text-ink-900">{selectedMapTicket.title}</p><p className="mt-0.5 truncate text-xs text-ink-450">{selectedMapTicket.property?.name} · {selectedMapTicket.property?.address}</p></Link></div><div className="flex gap-1.5 overflow-x-auto p-3">{mapTickets.slice(0, 6).map((ticket) => <button key={ticket.id} type="button" onClick={() => setSelectedMapId(ticket.id)} className={`shrink-0 rounded-full px-2.5 py-1.5 text-xs font-semibold transition ${ticket.id === selectedMapTicket.id ? "bg-petroleum-900 text-white" : ticket.priority === "urgent" ? "bg-danger-50 text-danger-700" : "bg-sand-50 text-ink-600 hover:bg-sand-100"}`}>{ticket.property?.name || "Ärende"}</button>)}</div></> : <EmptyState title="Inga kartlagda ärenden" description="När aktiva ärenden är kopplade till en fastighet visas platsen här." />}
      </article>
    </section>

    <section className="grid gap-4 xl:grid-cols-[1.65fr_0.75fr]">
      <article id="senastearenden" className="scroll-mt-36 overflow-hidden rounded-2xl border border-sand-200 bg-white shadow-premium-sm">
        <div className="flex items-center justify-between px-5 py-4"><div><h2 className="font-display text-[17px] font-semibold text-ink-950">Senaste ärenden</h2><p className="mt-0.5 text-xs text-ink-450">Klicka på ett ärende för historik, kommentarer och handläggning.</p></div><button id="senastearenden-alla" type="button" onClick={clearFilters} className="inline-flex items-center gap-1 text-xs font-semibold text-petroleum-700">Visa alla <ArrowRight className="h-3 w-3" /></button></div>
        {loading ? <p className="p-5 text-sm text-ink-500">Ärendena hämtas.</p> : recent.length ? <div className="overflow-x-auto"><table className="w-full min-w-[760px] text-left"><thead><tr className="border-y border-sand-100 bg-surface-subtle text-xs font-semibold uppercase tracking-[0.08em] text-ink-400"><th className="px-5 py-2.5">Ärende</th><th className="px-3">Fastighet</th><th className="px-3">Kategori</th><th className="px-3">Prioritet</th><th className="px-3">Status</th><th className="px-3">Tilldelad</th><th className="px-3">Skapad</th><th className="w-8" /></tr></thead><tbody>{recent.map((ticket) => <tr key={ticket.id} className="border-b border-sand-100 text-xs transition hover:bg-sand-50/60"><td className="px-5 py-3"><Link href={`/dashboard/felanmalan/${ticket.id}`} className="block max-w-[180px] truncate font-semibold text-ink-850">{ticket.title}</Link><span className="text-xs text-ink-400">#{ticket.id.slice(0, 8).toUpperCase()}</span></td><td className="px-3"><span className="block max-w-[140px] truncate font-medium text-ink-650">{ticket.property?.name || "—"}</span><span className="block max-w-[140px] truncate text-xs text-ink-400">{ticket.property?.address || "Ingen fastighet"}</span></td><td className="px-3 text-ink-550">{categoryLabels[ticket.category] || ticket.category}</td><td className="px-3"><span className={`rounded-full px-2 py-1 font-semibold ring-1 ${priorityTone(ticket.priority)}`}>{priorityLabels[ticket.priority] || ticket.priority}</span></td><td className="px-3"><span className={`rounded-full px-2 py-1 font-semibold ring-1 ${statusTone(ticket.status)}`}>{statusLabels[ticket.status] || ticket.status}</span></td><td className="max-w-[120px] truncate px-3 text-ink-550">{ticket.assigned_to?.name || ticket.assigned_to?.email || "Ej tilldelad"}</td><td className="whitespace-nowrap px-3 text-ink-450">{dateTime.format(new Date(ticket.created_at))}</td><td className="pr-3"><Link href={`/dashboard/felanmalan/${ticket.id}`} aria-label={`Öppna ${ticket.title}`}><ChevronRight className="h-4 w-4 text-ink-350" /></Link></td></tr>)}</tbody></table></div> : <EmptyState title="Inga ärenden i urvalet" description="Ändra filtren eller skapa ett nytt ärende." />}
        <div className="flex items-center justify-between gap-4 border-t border-sand-100 px-5 py-3"><p className="text-xs text-ink-450">Visar {visibleTickets.length} av {pagination.total.toLocaleString("sv-SE")} träffar</p>{pagination.totalPages > 1 ? <div className="flex gap-1.5"><button type="button" disabled={page <= 1} onClick={() => setPage((current) => Math.max(1, current - 1))} className="rounded-lg border border-sand-200 px-2.5 py-1.5 text-xs font-semibold text-ink-600 disabled:opacity-40">Föregående</button><button type="button" disabled={page >= pagination.totalPages} onClick={() => setPage((current) => Math.min(pagination.totalPages, current + 1))} className="rounded-lg border border-sand-200 px-2.5 py-1.5 text-xs font-semibold text-ink-600 disabled:opacity-40">Nästa</button></div> : null}</div>
      </article>

      <article className="rounded-2xl border border-sand-200 bg-white p-5 shadow-premium-sm">
        <div className="flex items-center justify-between"><div><h2 className="font-display text-[17px] font-semibold text-ink-950">Akuta ärenden</h2><p className="mt-0.5 text-xs text-ink-450">Aktiva ärenden med akut prioritet.</p></div><AlertTriangle className="h-4 w-4 text-danger-500" /></div>
        <div className="mt-4 divide-y divide-sand-100">{urgent.length ? urgent.map((ticket) => <Link key={ticket.id} href={`/dashboard/felanmalan/${ticket.id}`} className="flex items-center gap-3 py-3 transition hover:bg-sand-50/60"><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-danger-50 text-danger-600"><AlertTriangle className="h-4 w-4" /></span><div className="min-w-0 flex-1"><p className="truncate text-xs font-semibold text-ink-850">{ticket.title}</p><p className="mt-0.5 truncate text-xs text-ink-450">{ticket.property?.name || "Ingen fastighet"}</p></div><div className="text-right"><p className="text-xs font-semibold text-ink-550">{timeOnly.format(new Date(ticket.created_at))}</p><span className="mt-1 inline-block rounded-full bg-danger-50 px-2 py-0.5 text-xs font-semibold text-danger-700">Akut</span></div></Link>) : <p className="py-8 text-center text-xs text-ink-450">Inga akuta ärenden i urvalet.</p>}</div>
        <button type="button" onClick={() => { setPriorityFilter("urgent"); setPage(1); }} className="mt-4 inline-flex items-center gap-1 text-xs font-semibold text-petroleum-700">Visa alla akuta ärenden <ArrowRight className="h-3 w-3" /></button>
      </article>
    </section>

    {createOpen ? <div className="fixed inset-0 z-50 flex items-end justify-center bg-ink-950/25 p-3 backdrop-blur-[2px] sm:items-center sm:p-6" role="dialog" aria-modal="true" aria-labelledby="new-ticket-title"><div id="arende-editor" className="scroll-mt-36 max-h-[92vh] w-full max-w-xl overflow-y-auto rounded-3xl border border-sand-200 bg-surface-card shadow-premium-lg"><div className="flex items-start justify-between border-b border-sand-100 px-6 py-5"><div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-petroleum-700">Drift / Ärenden</p><h2 id="new-ticket-title" className="mt-1 font-display text-[24px] font-semibold text-ink-950">Nytt ärende</h2><p className="mt-1 text-xs text-ink-500">Registrera ärendet och gå direkt vidare till handläggningen.</p></div><button type="button" onClick={() => closeCreate()} className="flex h-9 w-9 items-center justify-center rounded-full border border-sand-200 bg-white text-ink-500 hover:bg-sand-50" aria-label="Stäng"><X className="h-4 w-4" /></button></div><form onSubmit={handleCreate} className="space-y-4 p-6"><label className="block"><span className="mb-1.5 block text-xs font-semibold text-ink-650">Fastighet</span><select value={propertyId} onChange={(event) => setPropertyId(event.target.value)} className={premiumFieldClass}><option value="">Ingen vald fastighet</option>{properties.map((property) => <option key={property.id} value={property.id}>{property.name} · {property.address}</option>)}</select></label><div className="grid gap-3 sm:grid-cols-2"><label className="block"><span className="mb-1.5 block text-xs font-semibold text-ink-650">Kategori</span><select value={category} onChange={(event) => setCategory(event.target.value)} className={premiumFieldClass}>{Object.entries(categoryLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label className="block"><span className="mb-1.5 block text-xs font-semibold text-ink-650">Prioritet</span><select value={priority} onChange={(event) => setPriority(event.target.value)} className={premiumFieldClass}>{Object.entries(priorityLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label></div><label className="block"><span className="mb-1.5 block text-xs font-semibold text-ink-650">Tilldelad</span><select value={assignedToId} onChange={(event) => setAssignedToId(event.target.value)} className={premiumFieldClass}><option value="">Ej tilldelad</option>{members.map((member) => <option key={member.id} value={member.id}>{member.name || member.email}</option>)}</select></label><label className="block"><span className="mb-1.5 block text-xs font-semibold text-ink-650">Titel</span><input id="arende-titel" required minLength={3} maxLength={180} value={title} onChange={(event) => setTitle(event.target.value)} className={premiumFieldClass} autoFocus placeholder="Exempel: Vattenläcka i badrum" /></label><label className="block"><span className="mb-1.5 block text-xs font-semibold text-ink-650">Beskrivning</span><textarea required minLength={10} maxLength={10000} value={description} onChange={(event) => setDescription(event.target.value)} className={premiumTextareaClass} placeholder="Beskriv problemet, platsen och vad som behöver göras." /></label><div className="flex justify-end gap-2 border-t border-sand-100 pt-4"><button type="button" onClick={() => closeCreate()} className="h-10 rounded-xl border border-sand-200 bg-white px-4 text-xs font-semibold text-ink-600">Avbryt</button><button type="submit" disabled={submitting} className="h-10 rounded-xl bg-petroleum-900 px-5 text-xs font-semibold text-white disabled:opacity-60">{submitting ? "Skapar…" : "Skapa ärende"}</button></div></form></div></div> : null}
  </div>;
}
