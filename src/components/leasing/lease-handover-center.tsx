...
import { HandoverWorkOrderPanel } from "@/components/leasing/handover-work-order-panel";
...
type Detail = { lease: LeaseOption; handover: LeaseHandoverPayload; permissions: { canManage: boolean }; history: AuditItem[] };
  ...
  return <Panel title="Inflyttning, avflyttning och nycklar" description="Checklista, nyckelkvittens och besiktning med spårbar historik.">
    <div className="space-y-6">
      ...
      {loading ? <div className="h-52 animate-pulse rounded-2xl bg-sand-100" /> : !detail ? <EmptyState title="Välj ett avtal" description="Pågående och avslutade avtal kan hanteras här." /> : <>
        ...
        <section className="rounded-2xl border border-sand-200 p-5"><div className="flex gap-2"><button type="button" onClick={() => update({ mode: "move_in" })} className={`rounded-lg px-3 py-2 text-sm font-semibold ${detail.handover.mode === "move_in" ? "bg-petroleum-700 text-white" : "bg-sand-100 text-ink-700"}`}>Inflyttning</button><button type="button" onClick={() => update({ mode: "move_out" })} className={`rounded-lg px-3 py-2 text-sm font-semibold ${detail.handover.mode === "move_out" ? "bg-petroleum-700 text-white" : "bg-sand-100 text-ink-700"}`}>Avflyttning</button></div><div className="mt-4 grid gap-2 md:grid-cols-2">{handoverChecklistKeys.map((key) => <label key={key} className="flex items-center gap-3 rounded-xl border border-sand-100 px-3 py-2.5 text-sm text-ink-700"><input type="checkbox" checked={detail.handover.checklist[key]} onChange={(event) => update({ checklist: { ...detail.handover.checklist, [key]: event.target.checked } })} className="h-4 w-4 accent-petroleum-700" />{labels[key]}{required?.includes(key) ? <span className="ml-auto text-[10px] font-semibold uppercase text-petroleum-700">Obligatorisk</span> : null}</label>)}</div></section>
        <HandoverKeyRegister keys={detail.handover.keys} onChange={(keys) => update({ keys })} />
        <section className="rounded-2xl border border-sand-200 p-5"><h3 className="font-semibold text-ink-900">Besiktning</h3><div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4"><select className={premiumFieldClass} value={detail.handover.inspection.status} onChange={(event) => updateInspection({ status: event.target.value as HandoverInspection["status"] })}><option value="not_started">Ej påbörjad</option><option value="scheduled">Planerad</option><option value="completed">Genomförd</option><option value="approved">Godkänd</option></select><input type="datetime-local" className={premiumFieldClass} value={detail.handover.inspection.scheduledAt?.slice(0,16) || ""} onChange={(event) => updateInspection({ scheduledAt: event.target.value || null })} /><input type="datetime-local" className={premiumFieldClass} value={detail.handover.inspection.completedAt?.slice(0,16) || ""} onChange={(event) => updateInspection({ completedAt: event.target.value || null })} /><input placeholder="Besiktningsperson" className={premiumFieldClass} value={detail.handover.inspection.inspector} onChange={(event) => updateInspection({ inspector: event.target.value })} /><select className={premiumFieldClass} value={detail.handover.inspection.condition} onChange={(event) => updateInspection({ condition: event.target.value as HandoverInspection["condition"] })}><option value="not_assessed">Ej bedömd</option><option value="approved">Godkänd</option><option value="remarks">Anmärkningar</option><option value="action_required">Åtgärd krävs</option></select><textarea placeholder="Besiktningsanteckning" className={`${premiumTextareaClass} md:col-span-2 xl:col-span-3`} value={detail.handover.inspection.note} onChange={(event) => updateInspection({ note: event.target.value })} /></div></section>
        <HandoverWorkOrderPanel leaseId={detail.lease.id} handover={detail.handover} />
        <textarea placeholder="Övergripande anteckning" className={premiumTextareaClass} value={detail.handover.generalNote} onChange={(event) => update({ generalNote: event.target.value })} />
        {detail.permissions.canManage ? <div className="flex flex-col gap-3 sm:flex-row sm:justify-end"><button type="button" disabled={saving} onClick={() => void save(false)} className="rounded-xl border border-petroleum-700 px-4 py-2.5 text-sm font-semibold text-petroleum-800">Spara utkast</button><button type="button" disabled={saving || Boolean(detail.handover.completedAt)} onClick={() => void save(true)} className={premiumPrimaryButtonClass}>{detail.handover.completedAt ? "Överlämning slutförd" : "Slutför överlämning"}</button></div> : <InlineAlert tone="info">Du har läsbehörighet.</InlineAlert>}
        <section className="rounded-2xl border border-sand-200 p-5"><div className="flex items-center gap-2"><History className="h-5 w-5 text-petroleum-700" /><h3 className="font-semibold text-ink-900">Historik</h3></div>{detail.history.length === 0 ? <p className="mt-3 text-sm text-ink-500">Ingen historik registrerad ännu.</p> : <div className="mt-3 divide-y divide-sand-100">{detail.history.map((item) => <div key={item.id} className="py-3 text-sm"><p className="font-medium text-ink-800">{item.action === "lease_handover.completed" ? "Överlämning slutförd" : item.action === "lease_handover.work_order_created" ? "Arbetsorder skapad från besiktning" : "Överlämning uppdaterad"}</p><p className="mt-1 text-xs text-ink-500">{date.format(new Date(item.created_at))} · {item.actor?.name || item.actor?.email || "System"}</p></div>)}</div>}</section>
      </>}
    </div>
  </Panel>;
}
