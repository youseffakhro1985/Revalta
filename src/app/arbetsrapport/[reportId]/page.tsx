    ...
    <main className="min-h-screen bg-sand-50 px-4 py-8 text-ink-950 print:bg-white print:p-0">
      ...
      <article className="mx-auto max-w-5xl rounded-3xl border border-sand-200 bg-white p-7 shadow-premium-sm print:max-w-none print:rounded-none print:border-0 print:p-0 print:shadow-none sm:p-10">
        ...
        <section className="grid gap-4 border-b border-sand-200 py-8 sm:grid-cols-2 lg:grid-cols-4">
          <div><p className="text-xs uppercase tracking-wide text-ink-500">Fastighet</p><p className="mt-2 font-semibold">{workOrder?.property?.name || "–"}</p><p className="mt-1 text-sm text-ink-500">{workOrder?.property ? `${workOrder.property.address}, ${workOrder.property.city}` : ""}</p></div>
          <div><p className="text-xs uppercase tracking-wide text-ink-500">Objekt</p><p className="mt-2 font-semibold">{workOrder?.unit?.designation || "Ej angivet"}</p></div>
          <div><p className="text-xs uppercase tracking-wide text-ink-500">Ansvarig</p><p className="mt-2 font-semibold">{workOrder?.assigned_to?.name || workOrder?.assigned_to?.email || "Ej tilldelad"}</p></div>
          <div><p className="text-xs uppercase tracking-wide text-ink-500">Rapporterad tid</p><p className="mt-2 font-semibold">{hours} h {minutes} min</p></div>
        </section>
