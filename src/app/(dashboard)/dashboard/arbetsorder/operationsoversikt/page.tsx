...
export default function WorkOrderOperationsPage() {
  ...
  useEffect(() => {
    if (loading) return;
    if (window.location.hash !== "#operativko") return;
    document.getElementById("operativko")?.scrollIntoView({ behavior: "smooth", block: "start" });
    window.setTimeout(() => document.getElementById("operativko-uppdatera")?.focus(), 0);
  }, [loading, data]);
