...
export default function RecurringIncidentSlaReportPage() {
  ...
  useEffect(() => {
    ...
    window.setTimeout(() => document.getElementById("exportera-csv")?.focus(), 0);
  }, [loading]);
  useEffect(() => {
    if (loading) return;
  ...
  return <div className="space-y-8">
    ...
    <Panel title="Incidentdetaljer" description="Rapporten visar den senaste SLA-versionen inom vald period och aktuell incidentstatus." bodyClassName="p-0">
      ...
      {rows.length ? <div className="overflow-x-auto"><table className="min-w-[1180px] w-full text-left text-sm">
        ...
        <tbody className="divide-y divide-sand-100">{rows.map((row) => <tr key={row.notificationKey} className={row.activeBreach ? "bg-danger-50/40" : "bg-white"}>
          ...
          <td className="px-4 py-4"><p className="text-ink-700">{when(row.responseDueAt)}</p><p className={`mt-1 text-xs font-semibold ${row.responseMet === false ? "text-danger-700" : row.responseMet ? "text-success-700" : "text-ink-500"}`}>{row.responseMet === null ? "Ej mätt" : row.responseMet ? "Uppfyllt" : "Ej uppfyllt"}</p></td>
          ...
          <td className="px-4 py-4"><p className="text-ink-700">{when(row.resolutionDueAt)}</p><p className={`mt-1 text-xs font-semibold ${row.resolutionMet === false ? "text-danger-700" : row.resolutionMet ? "text-success-700" : "text-ink-500"}`}>{row.resolutionMet === null ? "Ej mätt" : row.resolutionMet ? "Uppfyllt" : "Ej uppfyllt"}</p></td>
          ...
          <td className="px-4 py-4">{row.activeBreach ? <span className="rounded-full bg-danger-100 px-2.5 py-1 text-xs font-semibold text-danger-800">Aktivt SLA-brott</span> : <span className="rounded-full bg-success-50 px-2.5 py-1 text-xs font-semibold text-success-700">Ingen aktiv avvikelse</span>}</td>
        </tr>)}</tbody>
      </table></div> : null}
    </Panel>
    </div>
