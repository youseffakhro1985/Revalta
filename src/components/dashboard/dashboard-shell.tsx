...
function NavigationLink({
  ...
  return (
    <Link
      ...
      className={`group relative flex items-center gap-3 rounded-lg outline-none transition-[background-color,color] duration-200 ease-out focus-visible:ring-2 focus-visible:ring-white/55 focus-visible:ring-offset-2 focus-visible:ring-offset-petroleum-900 ${compact ? "min-h-9 py-1.5 pl-3 pr-2.5 text-[13px]" : "min-h-10 py-2 pl-3 pr-2.5 text-[13.5px]"} ${active ? "bg-white/[0.10] font-medium text-white" : "font-normal text-white/70 hover:bg-white/[0.055] hover:text-white"}`}
    >
      {active ? <span className="absolute inset-y-1.5 left-0 w-[2px] rounded-full bg-sand-200/85" aria-hidden="true" /> : null}
      <Icon className={`${compact ? "h-4 w-4" : "h-[17px] w-[17px]"} shrink-0 ${active ? "text-white" : "text-white/60 transition-colors group-hover:text-white"}`} strokeWidth={1.7} aria-hidden="true" />
...
function NavigationContent({
  ...
  if (resident) {
    return (
      <div>
        <p className="mb-2 px-3 text-[11px] font-medium uppercase tracking-[0.12em] text-white/45">Min portal</p>
  ...
  return (
    <>
      <div>
        <p className="mb-2 px-3 text-[11px] font-medium uppercase tracking-[0.12em] text-white/45">Arbetsyta</p>
        <div className="space-y-0.5">
          {primaryItems.map((item) => <NavigationLink key={item.href} item={item} pathname={pathname} onNavigate={onNavigate} />)}
        </div>
      </div>

      <div className="mt-6 space-y-0.5" aria-label="Modulområden">
          ...
          return (
            <div key={section.id}>
              <button
                ...
                className={`group flex min-h-10 w-full items-center gap-3 rounded-lg px-3 text-left text-[13.5px] outline-none transition-[background-color,color] duration-200 focus-visible:ring-2 focus-visible:ring-white/55 focus-visible:ring-offset-2 focus-visible:ring-offset-petroleum-900 ${sectionActive ? "font-medium text-white" : "font-normal text-white/70 hover:bg-white/[0.055] hover:text-white"}`}
              >
                <Icon className={`h-[17px] w-[17px] shrink-0 ${sectionActive ? "text-white" : "text-white/60 group-hover:text-white"}`} strokeWidth={1.7} aria-hidden="true" />
                <span className="min-w-0 flex-1 truncate">{section.label}</span>
                <ChevronDown className={`h-3.5 w-3.5 shrink-0 text-white/40 transition-transform duration-200 ${expanded ? "rotate-180" : ""}`} strokeWidth={1.7} aria-hidden="true" />
              </button>
      ...
      <div className="mt-6 border-t border-white/10 pt-4">
        <p className="mb-2 px-3 text-[11px] font-medium uppercase tracking-[0.12em] text-white/45">Administration</p>
      ...
      <aside className="fixed inset-y-0 left-0 z-40 hidden w-[264px] overflow-hidden border-r border-petroleum-900/35 bg-petroleum-900 text-white lg:flex lg:flex-col">
        <div className="relative flex h-16 items-center border-b border-white/10 px-5">
          <Link href={homeHref} className="rounded-md outline-none focus-visible:ring-2 focus-visible:ring-white/60" aria-label="Revalta dashboard">
            <span className="font-display text-[22px] font-semibold tracking-[-0.04em] text-white">REVALTA</span>
            <span className="mt-0.5 block text-[10px] font-medium uppercase tracking-[0.18em] text-white/40">Fastighetsförvaltning</span>
          </Link>
        </div>
        <nav aria-label="Dashboardmeny" className="relative flex-1 overflow-y-auto px-3 py-5"><NavigationContent pathname={pathname} role={role} /></nav>
        <div className="relative border-t border-white/10 p-4">
          <div className="mb-2 flex items-center gap-3 rounded-lg px-3 py-2.5">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-0.5 truncate text-[11.5px] text-white/50">{roleLabel}</p>
            </div>
          </div>
