...
export default function WorkOrdersPage() {
  ...
  useEffect(() => {
    ...
    window.setTimeout(() => document.getElementById("senasteordrar-alla")?.focus(), 0);
  }, [loading, orders]);
  useEffect(() => {
    if (loading) return;
    ...
    window.setTimeout(() => document.getElementById("dagensplanering-alla")?.focus(), 0);
  }, [loading, orders]);
  ...
  return <div className="space-y-4 sm:space-y-5">
    <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
      <div>
        <p className="text-xs font-semibold uppercase tracking-[0.18em] text-petroleum-700">Operativ förvaltning / Arbetsorder</p>
        ...
        {scopedToAssigned ? <p className="mt-2 text-xs font-semibold text-petroleum-700">Vyn är begränsad till arbetsordrar som är tilldelade dig.</p> : null}
      </div>
    ...
    <section className="grid gap-4 xl:grid-cols-[1.2fr_0.68fr_0.82fr]">
      <article className="rounded-2xl border border-sand-200 bg-white p-5 shadow-premium-sm">
        <div className="flex items-center justify-between gap-3"><div><h2 className="font-display text-[18px] font-semibold text-ink-950">Arbetsorderutveckling</h2><p className="mt-1 text-xs text-ink-450">Verkliga arbetsorder grupperade per vecka.</p></div><Link href="/dashboard/arbetsorder/operationsoversikt" className="rounded-lg border border-sand-200 bg-white px-3 py-2 text-xs font-semibold text-petroleum-700 hover:bg-petroleum-50">Veckovis</Link></div>
        <div className="mt-4 flex flex-wrap gap-4 text-xs text-ink-500"><Legend color="#29463f" text="Skapade" /><Legend color="#779e90" text="Planerade/pågående" /><Legend color="#c6b17b" text="Slutförda" /><Legend color="#dc2626" text="Försenade nu" /></div>
        <div className="mt-4 rounded-xl bg-surface-subtle px-3 py-4">
          ...
          <div className="grid grid-cols-8 text-center text-xs text-ink-400">{weekly.map((point) => <span key={point.label}>{point.label}</span>)}</div>
        </div>
        <Link href="/dashboard/arbetsorder/operationsoversikt" className="mt-4 inline-flex items-center gap-1 text-xs font-semibold text-petroleum-700 hover:text-petroleum-900">Visa detaljerad rapport <ArrowRight className="h-3 w-3" /></Link>
      </article>

      <article className="rounded-2xl border border-sand-200 bg-white p-5 shadow-premium-sm">
        <div className="flex items-center justify-between gap-3"><h2 className="font-display text-[18px] font-semibold text-ink-950">Statusfördelning</h2><button type="button" onClick={() => setFocus("all")} className="rounded-lg border border-sand-200 bg-white px-3 py-2 text-xs font-semibold text-ink-600 hover:bg-sand-50">Alla arbetsorder</button></div>
        <div className="mt-5 divide-y divide-sand-100">{statusRows.map((row) => <div key={row.status} className="grid grid-cols-[minmax(0,1fr)_72px_42px] items-center gap-3 py-3"><div className="min-w-0"><div className="flex items-center gap-2"><span className={`h-2 w-2 rounded-full ${row.status === "completed" ? "bg-success-500" : row.status === "in_progress" ? "bg-warning-500" : row.status === "waiting_material" || row.status === "blocked" ? "bg-violet-500" : "bg-sky-500"}`} /><span className="truncate text-xs font-semibold text-ink-700">{statusLabels[row.status] || row.status}</span></div><div className="mt-2 h-1.5 overflow-hidden rounded-full bg-sand-100"><div className="h-full rounded-full bg-petroleum-700" style={{ width: `${Math.max(row.count ? 8 : 0, row.percent)}%` }} /></div></div><span className="text-right text-xs font-semibold text-ink-700">{row.count}</span><span className="text-right text-xs text-ink-400">{row.percent}%</span></div>)}</div>
        <button type="button" onClick={() => setFocus("all")} className="mt-4 inline-flex items-center gap-1 text-xs font-semibold text-petroleum-700 hover:text-petroleum-900">Visa alla statusar <ArrowRight className="h-3 w-3" /></button>
      </article>

      <article className="overflow-hidden rounded-2xl border border-sand-200 bg-white shadow-premium-sm">
        <div className="flex items-center justify-between px-4 py-3.5"><div><h2 className="font-display text-[17px] font-semibold text-ink-950">Arbetsorder på karta</h2><p className="mt-0.5 text-xs text-ink-450">Klicka mellan arbetsorder och planera rutt.</p></div><MapPin className="h-4 w-4 text-petroleum-700" /></div>
        ...
        {selectedMapOrder ? <>
          <div className="relative h-[225px] bg-sand-100">
            ...
            <div className="pointer-events-none absolute left-3 top-3 max-w-[220px] rounded-xl border border-white/80 bg-white/95 px-3 py-2 shadow-premium-sm backdrop-blur-sm"><p className="text-xs font-semibold text-ink-900">{workOrderNumber(selectedMapOrder)}</p><p className="mt-0.5 truncate text-xs text-ink-500">{selectedMapOrder.property.name} · {selectedMapOrder.property.address}</p><p className="mt-1 text-xs font-semibold text-petroleum-700">{priorityLabels[selectedMapOrder.priority] || selectedMapOrder.priority} · {selectedMapOrder.title}</p></div>
          </div>
          <div className="border-t border-sand-100 p-3">
            {mapOrders.length > 1 ? <select value={selectedMapOrder.id} onChange={(event) => setSelectedMapId(event.target.value)} aria-label="Välj arbetsorder på kartan" className="mb-2.5 h-9 w-full rounded-xl border border-sand-200 bg-surface-subtle px-3 text-xs font-semibold text-ink-650 outline-none focus:ring-2 focus:ring-petroleum-100">{mapOrders.map((order) => <option key={order.id} value={order.id}>{workOrderNumber(order)} · {order.property.name}</option>)}</select> : null}
            <div className="grid grid-cols-2 gap-2"><a href={mapExternal} target="_blank" rel="noreferrer" className="inline-flex h-9 items-center justify-center gap-1.5 rounded-xl border border-sand-200 bg-white px-2 text-xs font-semibold text-petroleum-700 hover:bg-petroleum-50">Öppna karta <ExternalLink className="h-3 w-3" /></a><a href={routeUrl(mapOrders)} target="_blank" rel="noreferrer" className="inline-flex h-9 items-center justify-center gap-1.5 rounded-xl bg-petroleum-900 px-2 text-xs font-semibold text-white hover:bg-petroleum-800"><Route className="h-3.5 w-3.5" />Planera rutt</a></div>
          </div>
        </> : <div className="flex h-[290px] flex-col items-center justify-center px-6 text-center"><MapPin className="h-7 w-7 text-sand-400" /><p className="mt-2 text-xs font-semibold text-ink-700">Ingen arbetsorder med plats i filtret.</p><button type="button" onClick={() => { setFocus("all"); setQuery(""); }} className="mt-3 text-xs font-semibold text-petroleum-700">Rensa filter</button></div>}
      </article>
    </section>
    ...
    <section className="grid gap-4 xl:grid-cols-[1.24fr_0.78fr_0.86fr]">
      <article id="senasteordrar" className="scroll-mt-36 overflow-hidden rounded-2xl border border-sand-200 bg-white shadow-premium-sm">
        <SectionHead title="Senaste arbetsorder" action={<button id="senasteordrar-alla" type="button" onClick={() => { setFocus("all"); setQuery(""); }} className="inline-flex items-center gap-1 text-xs font-semibold text-petroleum-700">Visa alla <ArrowRight className="h-3 w-3" /></button>} />
      ...
      <article id="dagensplanering" className="scroll-mt-36 rounded-2xl border border-sand-200 bg-white shadow-premium-sm">
        <SectionHead title="Dagens planering" action={<Link id="dagensplanering-alla" href="/dashboard/arbetsorder/planering" className="inline-flex items-center gap-1 text-xs font-semibold text-petroleum-700">Visa alla <ArrowRight className="h-3 w-3" /></Link>} />
...
function MapChip({ label, dot, onClick }: { label: string; dot: string; onClick: () => void }) {
  return <button type="button" onClick={onClick} className="inline-flex items-center gap-1.5 rounded-full border border-sand-200 bg-white px-2.5 py-1 text-xs font-semibold text-ink-650 hover:bg-sand-50"><span className={`h-2 w-2 rounded-full ${dot}`} />{label}</button>;
}
...
function Empty({ title }: { title: string }) {
  return <div className="flex min-h-40 flex-col items-center justify-center px-5 text-center"><BadgeCheck className="h-6 w-6 text-sand-400" /><p className="mt-2 text-xs font-semibold text-ink-650">{title}</p></div>;
}
