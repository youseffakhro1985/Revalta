...
export function LeaseInspectionItemsCenter() {
  ...
  function removeItem(id: string) {
    if (!detail || !window.confirm("Ta bort besiktningspunkten?")) return;
    ...
    <div className="space-y-5">
      ...
      {loading ? <div className="h-48 animate-pulse rounded-xl bg-sand-100" /> : !detail ? <EmptyState title="Välj ett avtal" description="Besiktningspunkter registreras separat per avtal och objekt." /> : <>
        ...
        <div className="space-y-4">{detail.record.items.length === 0 ? <EmptyState title="Inga besiktningspunkter" description="Lägg till rum, område eller byggnadsdel som ska kontrolleras." /> : detail.record.items.map((item, index) => <article key={item.id} className="rounded-2xl border border-sand-200 p-5">
          <div className="flex items-center justify-between"><h3 className="font-semibold text-ink-900">Besiktningspunkt {index + 1}</h3>{detail.permissions.canManage ? <button type="button" onClick={() => removeItem(item.id)} className="rounded-lg p-2 text-ink-400 hover:bg-red-50 hover:text-red-700" aria-label="Ta bort"><Trash2 className="h-4 w-4" /></button> : null}</div>
          <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4"><input className={premiumFieldClass} placeholder="Rum eller område" value={item.area} onChange={(event) => patchItem(item.id, { area: event.target.value })} /><input className={premiumFieldClass} placeholder="Byggnadsdel, t.ex. golv" value={item.component} onChange={(event) => patchItem(item.id, { component: event.target.value })} /><select className={premiumFieldClass} value={item.condition} onChange={(event) => patchItem(item.id, { condition: event.target.value as InspectionCondition, selectedForWorkOrder: event.target.value === "action_required" ? item.selectedForWorkOrder : false })}>{Object.entries(conditionLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><select className={premiumFieldClass} value={item.priority} onChange={(event) => patchItem(item.id, { priority: event.target.value as InspectionPriority })}>{Object.entries(priorityLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></div>
          <div className="mt-3 grid gap-3 md:grid-cols-2"><textarea className={premiumTextareaClass} placeholder="Beskriv skick eller anmärkning" value={item.description} onChange={(event) => patchItem(item.id, { description: event.target.value })} /><textarea className={premiumTextareaClass} placeholder="Rekommenderad åtgärd" value={item.recommendation} onChange={(event) => patchItem(item.id, { recommendation: event.target.value })} /></div>
          <div className="mt-3 flex flex-wrap gap-4 text-sm text-ink-700"><label className="flex items-center gap-2"><input type="checkbox" checked={item.selectedForWorkOrder} disabled={item.condition !== "action_required"} onChange={(event) => patchItem(item.id, { selectedForWorkOrder: event.target.checked })} className="h-4 w-4 accent-petroleum-700" />Välj för arbetsorder</label><label className="flex items-center gap-2"><input type="checkbox" checked={item.resolved} onChange={(event) => patchItem(item.id, { resolved: event.target.checked })} className="h-4 w-4 accent-petroleum-700" />Åtgärdad</label></div>
        </article>)}</div>
        {detail.permissions.canManage ? <div className="flex flex-col gap-3 sm:flex-row sm:justify-between"><button type="button" onClick={() => setItems([...detail.record.items, newItem()])} className="inline-flex items-center justify-center rounded-xl border border-petroleum-700 px-4 py-2.5 text-sm font-semibold text-petroleum-800"><Plus className="mr-2 h-4 w-4" />Lägg till besiktningspunkt</button><button type="button" disabled={saving} onClick={() => void save()} className={premiumPrimaryButtonClass}><ClipboardCheck className="mr-2 h-4 w-4" />{saving ? "Sparar…" : "Spara besiktning"}</button></div> : <InlineAlert tone="info">Du har läsbehörighet.</InlineAlert>}
      </>}
    </div>
  </Panel>;
}
