...
import { AlertTriangle, BellRing, Check, ChevronRight, ShieldAlert } from "lucide-react";
...
export function NotificationMenu() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const [serviceResponse, securityResponse] = await Promise.all([
      ...
      setData({
        notifications: [...securityUnread, ...serviceUnread],
        summary: {
          total: (security?.summary.total || 0) + (service?.summary.total || 0),
          unread: securityUnread.length + (service?.summary.unread || 0),
          overdue: service?.summary.overdue || 0,
          high: (security?.summary.high || 0) + (service?.summary.high || 0),
        },
      });
    } finally {
      setLoading(false);
  ...
  async function markRead(item: Notification) {
    const endpoint = item.kind === "security" ? "/api/notifications/work-order-locks" : "/api/notifications/service-center";
      ...
      {open ? (
        <div className="absolute right-0 top-[calc(100%+10px)] z-50 w-[min(92vw,390px)] overflow-hidden rounded-2xl border border-sand-200 bg-white shadow-2xl">
          <div className="flex items-center justify-between border-b border-sand-100 px-5 py-4">
            <div><p className="font-semibold text-ink-950">Aviseringar</p><p className="mt-0.5 text-xs text-ink-500">{unread} olästa aviseringar</p></div>
          ...
          <div className="max-h-[430px] overflow-y-auto">
            ...
            <div className="divide-y divide-sand-100">
              {preview.map((item) => {
                const Icon = item.kind === "security" ? ShieldAlert : AlertTriangle;
                return <div key={item.key} className="p-4 hover:bg-sand-50/70">
                  <div className="flex gap-3">
                    <div className={`mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-xl ${item.kind === "security" ? "bg-red-50 text-red-700" : item.overdue ? "bg-red-50 text-red-700" : item.high ? "bg-amber-50 text-amber-700" : "bg-petroleum-50 text-petroleum-700"}`}><Icon className="h-4 w-4" /></div>
