...
Both branch-scoped Preview variables have now been saved after the final credential rotation: pooled `DATABASE_URL` and direct `DIRECT_URL`. This documentation-only commit intentionally triggers a fresh GitHub-linked Vercel Preview deployment on a new exact head SHA after both final saves. All required CI, CodeQL, Preview Health Attestation and Preview Browser E2E checks must be evaluated again against that exact SHA before merge.
