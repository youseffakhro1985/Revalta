...
export default function TechnicianPlanningPage() {
  ...
  useEffect(() => {
    ...
    if (loading) return;
    window.setTimeout(() => document.getElementById("fordela-arbete")?.focus(), 0);
  }, [loading, orders]);
  useEffect(() => {
    if (loading) return;
    ...
    window.setTimeout(() => document.getElementById("belastningslista-uppdatera")?.focus(), 0);
  }, [loading, orders]);
  ...
  return <div className="space-y-8">
    <PageHeader
      ...
      action={<div className="flex flex-wrap gap-2">{canAssign || loading ? <a id="fordela-arbete" autoFocus href="#arbetsbelastning" className="inline-flex h-11 items-center gap-2 rounded-xl bg-petroleum-800 px-4 text-sm font-semibold text-white hover:bg-petroleum-900">Fördela arbete</a> : null}<button id="belastningslista-uppdatera" type="button" onClick={() => void load()} disabled={loading} className="inline-flex h-11 items-center gap-2 rounded-xl border border-sand-200 bg-white px-4 text-sm font-semibold text-ink-700 hover:bg-sand-50 disabled:opacity-50"><RefreshCw className={`h-4 w-4 ${loading ? "animate-spin" : ""}`} /> Uppdatera</button></div>}
    />
