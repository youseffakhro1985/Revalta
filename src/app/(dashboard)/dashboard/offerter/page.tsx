...
export default function QuotesPage() {
  ...
  async function load() { setLoading(true); const response = await fetch("/api/quotes", { cache: "no-store" }); const data = await readResponseJson(response); if (response.ok) { setQuotes(data.quotes || []); setProperties(data.properties || []); setCanManage(Boolean(data.permissions?.canManage)); } else setError(data.error || "Kunde inte hämta offerter"); setLoading(false); }
  useEffect(() => { void load(); }, []);
