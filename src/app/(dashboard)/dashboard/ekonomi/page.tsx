...
export default function EconomyPage(){
  ...
  return <div className="space-y-4 sm:space-y-5">
    <div className="flex flex-col gap-3 xl:flex-row xl:items-end xl:justify-between"><div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-petroleum-700">Ekonomi & analys / Översikt</p><h1 className="mt-1 font-display text-[30px] font-semibold tracking-[-0.045em] text-ink-950 sm:text-[34px]">Ekonomi</h1><p className="mt-1 text-sm text-ink-500">Hyresintäkter, budget, utfall och betalningsläge i en samlad ekonomisk arbetsyta.</p></div><div className="flex gap-2"><select value={range} onChange={e=>setRange(e.target.value as RangeKey)} aria-label="Period" className="h-10 rounded-xl border border-sand-200 bg-white px-3 text-xs font-semibold text-ink-650"><option value="quarter">Senaste kvartalet</option><option value="year">Detta år</option><option value="all">Alla perioder</option></select>{canManage?<Link href="/dashboard/ekonomi/ny-utbetalning" className="inline-flex h-10 items-center gap-2 rounded-xl bg-petroleum-900 px-4 text-xs font-semibold text-white"><CircleDollarSign className="h-4 w-4"/>Ny utbetalning</Link>:null}</div></div>
    <section className="rounded-2xl border border-sand-200 bg-white p-3 shadow-premium-sm"><label className="relative block max-w-2xl"><Search className="absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-ink-400"/><input value={query} onChange={e=>setQuery(e.target.value)} aria-label="Sök i ekonomin" placeholder="Sök fakturor, hyresgäster, konton, budget ..." className="h-11 w-full rounded-xl border border-sand-200 bg-surface-subtle pl-10 pr-4 text-[12px] outline-none focus:ring-2 focus:ring-petroleum-100"/></label></section>
